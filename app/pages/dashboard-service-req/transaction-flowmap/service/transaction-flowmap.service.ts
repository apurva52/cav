...
//import { TRANSACTION_FLOWMAP_DATA2 } from './transaction-flowmap.dummy';
import { TransactionFlowMapLoadingState, TransactionFlowMapLoadedState } from './transaction-flowmap.state';
import { SessionService } from 'src/app/core/session/session.service';
import { environment } from 'src/environments/environment';
import { TransactionFlowMapServiceInterceptor } from './transaction-flowmap.service.interceptor';
...
export class TransactionFlowMapService extends Store.AbstractService {
  ...
  constructor(
   ...
   // private route: ActivatedRoute
    ...
    super();
    // this.interceptor = new TransactionFlowMapServiceInterceptor(this, sessionService);
  }
...
me.controller.post(path, payload).subscribe(
  (data) => {
   ...
   // output.next(new TransactionFlowMapLoadedState( this.interceptor.jsonData));

   output.next(new TransactionFlowMapLoadedState( this.jsonData ));
    output.complete();
  },
  (e: any) => {
    ...
    me.logger.error('Trx Flowmap Report Data loading failed', e);
  }
);
   /* setTimeout(() => {
