...
import { ServiceLogsService } from './service/service-logs.service';
...
export class ServiceLogsComponent implements OnInit {
  ...
  constructor(private serviceLog: ServiceLogsService  ,
  ...
  ngOnInit(): void {
    ...
    me.cols = me.dbMonService.loadTableHeaders('CONFIG','SERVICELOGS');
      me.dataTable = {
        paginator: {
          first: 1,
          rows: 10,
  ...
  private scheduleLoad(){
    ...
    if(me.schedulerService){
      me.schedulerService.unsubscribe('load-servicelog');
    }
    if(me.dbMonService.isAnalysisMode){
      return;
    }
    me.schedulerService.subscribe('load-servicelog', () => {
      me.load();
    }, 'progressInterval');
  }
  ...
  load() {
    ...
    if (!me.sessionSerivce.isActive()) {
      ...
      me.serviceLog.getPresetAndLoad().subscribe(
      (state: Store.State) => {
...
customSort(event) {
  event.data.sort((data1, data2) => {
    ...
    if (value1 == null && value2 != null)
      ...
      else if (event['field'] == 'timestamp' || event['field'] == 'dbName' || event['field'] == 'className' || event['field'] == 'methodName' || event['field'] == 'serviceName') 
