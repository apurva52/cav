...
import { BlockingSessionService } from './services/blocking-session.service';

@Component({
  selector: 'app-blocking-session',
  templateUrl: './blocking-session.component.html',
  styleUrls: ['./blocking-session.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class BlockingSessionComponent implements OnInit {
  ...
  private onLoaded(state: DBMonitoringLoadedState) {
    ...
    if (state.data['arrStartAndEndTimeForDB'][0] && state.data['arrStartAndEndTimeForDB'][1] && state.data['arrStartAndEndTimeForDB'][0] != null && state.data['arrStartAndEndTimeForDB'][1] != null) {
      if(me.dbMonService.isRealTimeAppled){
        me.timeRange = state.data['arrStartAndEndTimeForDB'][1];
      }else{
        me.timeRange = state.data['arrStartAndEndTimeForDB'][0] + ' to ' + state.data['arrStartAndEndTimeForDB'][1];
      }
    }
    if (state.data && state.data['totalBlockedProcessesWithTime']) {
      me.enableChart = false;
      let graphData = state.data['totalBlockedProcessesWithTime'];
      me.dataChart[0]['highchart']['series'] = [
        {
          name: 'Lead Processes',
          data: me.dbMonService.getTimedDataArray(graphData['arrTSOfBlockedProcess'], graphData['countOfLeadProcess'])
        },
        {
          name: 'Blocked Processes',
          data: me.dbMonService.getTimedDataArray(graphData['arrTSOfBlockedProcess'], graphData['countOfBlockedProcess'])
        }
