...
@Component({
  selector: 'take-thread-dump',
  templateUrl: 'take-thread-dump.component.html',
  styleUrls: ['take-thread-dump.component.scss']
})
export class TakeThreadDumpComponent implements OnDestroy{
  dcinfo: any;
  isActive=false;
  agent_Type: string;
...
defaultServerTime:Date;
  ...
  constructor(private commonServices: CommonServices, private _cavConfigService: CavConfigService,
    private _navService: CavTopPanelNavigationService, public _ddrData: DdrDataModelService,private breadcrumbService :DdrBreadcrumbService,private confirmationService: ConfirmationService
...
//	if (this.selectedTiers[0] != "-- Select Tier --" && this.selectedServers[0] != "-- Select Server --" && this.selectedApps[0] != "-- Select Instance --")
    ...
    view_thread_dump_url =  this.getHostUrl() + '/' + this.id.product + "/v1/cavisson/netdiagnostics/ddr/viewThreadDump?testRun=" + this.id.testRun + "&tierName=" + this.selectedTiers + "&serverName=" + this.selectedServers + "&instanceName=" + this.selectedApps + "&startTime=" + this.id.strStartTime + "&endTime=" + this.id.strEndTime;
...
getSummaryThreadDumps()
{
  ...
  showAnalysisThreadDump(data: any,agentType?:string) {
    ...
    if((this.agentType != undefined) && this.agentType.toLowerCase() !== 'java') {
      ...
      let sunstringFilePath = filePath.substring(filePath.indexOf("logs"), filePath.length);
      let ipWithProd = "";
      if(data.dcName && sessionStorage.getItem("isMultiDCMode") == "true" && this._cavConfigService.getActiveDC() == "ALL"){
      ipWithProd = this.getURLbasedOnDC(data.dcName) + '/' + this.id.product;        
      }
      else
      ipWithProd = this.getHostUrl() + '/' + this.id.product;
  ...
  takeThreadDump() {
    ...
    var url = this.getHostUrl() + '/' + this.id.product + "/v1/cavisson/netdiagnostics/ddr/getTierServerListFromTopology?testRun=" + this.id.testRun;
  ...
  getJavaInstances() {
    ...
    var url = ipWithProd + '/' + this.id.product + "/v1/cavisson/netdiagnostics/ddr/getJavaInstances?server=" + this.serverNameforTD + "&testRun="+testRun;
  ...
  getNDInstances() {
    ...
    var url =  ipWithProd + '/' + this.id.product + "/v1/cavisson/netdiagnostics/ddr/getNDInstances?server=" + this.serverNameforTD + "&tier=" + this.selectedTierVal;
    return this.ddrRequest.getDataUsingGet(url).subscribe(data => {
         ...
         if(this.TDTakencount > 1)
          {
           ...
           setTimeout(() => {
            ...
            clearInterval(takedump);
              ...
              let subscription = this.asyncTDaken.takeWhile(()=>this.alive).subscribe(
...
this.showTInterval=true;
  ...
  //this.timer+=1;
    ...
    if (selectedInstanceInfo.length > 0) {
      ...
      var restUrl = ipWithProd + '/' + this.id.product + "/v1/cavisson/netdiagnostics/ddr/takeThreadDump?testRun=" + testRun + "&tierName=" + selectedTier + "&serverName=" + serverNameforTD + "&instanceName=NA&startTime=NA&endTime=NA&dumpType=1&heapFilePath=NA" + "&processIdWithInstance=" + this.processIDWithInstance + "&isFromND=" + isNDCase + "&loginUserName=" + this.id.userName + "&isCompressed=" + compressedMode+"&agentType="+this.agent_Type;
  ...
  openDownloadReports(res:any,dcName?) {
    ...
    window.open(decodeURIComponent(ipWithProd + '/' + this.id.product).replace("/netstorm", "").replace("/netdiagnostics", "") + "/common/" + fileName);
  }
...
downloadFile(node:any)
{
  ...
  let url = ipWithProd + "/" + this.id.product + "/v1/cavisson/netdiagnostics/ddr/downloadThreadDump?filePath="+filePath;
...
let delete_TD_url = ipWithProd + '/' + this.id.product + "/v1/cavisson/netdiagnostics/ddr/removeThreadDumpFile?testRun=" + testRun+'&filePath='+node.filePath+'&index='+node.index + '&productKey=' + this.productKey + '&userName=' + this.userName ;
 ...
 // alert("endDate--------------"+this.endDate);
  ...
  if (this.selectedInstanceInfo.length > 0) {
   ...
   if(this.isChecked )
        ...
        this.isCompressed = 0;
  let schedule_Url=this.getHostUrl()+"/"+this.id.product+"/scheduleDumps?testRun=" + this.id.testRun + "&tierName=" + this.selectedTierVal + "&serverName=" +serverNameforTD + "&processIdWithInstance=" + this.processIDWithInstance+ "&isFromND=" + this.isNDCase + "&loginUserName=" + this.id.userName + "&isCompressed=" + this.isCompressed+"&count="+this.TDTakencount+"&interval="+timeInterval+"&strDate="+ this.strTimeInDateFormat+"&endDate="+this.endTimeInDateFormat+"&action=Scheduled";
...
{
  let get_schedule_td_Url=this.getHostUrl()+"/"+this.id.product+"/v1/cavisson/netdiagnostics/webddr/getScheduleTDList?testRun=" + this.id.testRun + "&userName="+this.id.userName+"&tierName="+this.selectedTiers+"&serverName="+this.selectedServers+"&instanceName="+this.selectedApps;
...
this.displayReschedulePopup=false;     
 this.displayPopUp=false;
  ...
  getServerTime(){
    try {
          let url = this.getHostUrl() + '/' + this.id.product + "/v1/cavisson/netdiagnostics/webddr/getServerCurrentTime" ;
