...
import { GroupByAllFrameInterface, hotspotSecInterface } from '../../../interfaces/group-by-all-data-info';
...
@Component({
  selector: 'app-group-by-all',
  templateUrl: './group-by-all.component.html',
  styleUrls: ['./group-by-all.component.css']
})
export class GroupByAllComponent implements OnInit {
 ...
 //variable used for disable/enable loader image
  ...
  hotSpotInfo: Array<GroupByAllFrameInterface>;
  ...
  isEnabledColumnFilterForSignature = true;
  ...
  */
  ngOnInit() {
    ...
    this.createCols();
    console.log("oninit call for all frame");
  ...
  createCols() {
    ...
    this.colsForSignature = [
      ...
      { field: 'TopMethods', header: 'StackTrace(Top Methods)', sortable: true, action: true, align: 'left', color: 'black', width: '271' },
      { field: 'CompareDepth', header: 'Compare Depth', sortable: true, action: true, align: 'right', color: 'black', width: '95' },
      { field: 'MinDepth', header: 'Min Depth', sortable: 'custom', action: true, align: 'right', color: 'black', width: '95' },
      { field: 'MaxDepth', header: 'Max Depth', sortable: 'custom', action: true, align: 'right', color: 'black', width: '95' },
  ...
  ngOnDestroy()
  {
    this.commonService.indexArr.push(1);
  }

  //constructor  to initialize value
  constructor( private commonService: CommonServices, private route: ActivatedRoute, private router: Router, private _cavConfigService: CavConfigService, private _navService: CavTopPanelNavigationService,private _ddrData:DdrDataModelService,
    private ddrRequest:DDRRequestService) {  
    console.log("Constructor call for all frame");
  }
  ...
  clickApply(searchVal, timeVal) {
    // console.log("searchVal --- " + searchVal + " , timeVal ----" ,timeVal);
    ...
    if ((searchVal == undefined) && (timeVal == undefined)) {
      ...
      // else
        ...
        if(this.commonService.enableQueryCaching == 1){
          endpoint_url += '/' + this.id.product.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/ASGroupFrameData?cacheId="+ this.id.testRun + "&testRun=" + this.id.testRun + "&tierName=" + this.id.tierName + "&serverName=" + this.id.serverName + "&appName=" + this.id.appName + "&strStartTime=" + strStartTime + "&strEndTime=" + strEndTime + "&searchKeyVal=" + searchVal;
        }
        else{
          endpoint_url += '/' + this.id.product.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/ASGroupFrameData?testRun=" + this.id.testRun + "&tierName=" + this.id.tierName + "&serverName=" + this.id.serverName + "&appName=" + this.id.appName + "&strStartTime=" + strStartTime + "&strEndTime=" + strEndTime + "&searchKeyVal=" + searchVal;
        }
  ...
  //Method called when click on Thread count row
...
*/
  ...
  */
  ...
  passDataForStackTrace(data: any) {
    ...
    if(this.commonService.enableQueryCaching == 1){
      second_url+= '/' + this.id.product.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/ASStackTraceData?cacheId="+ this.id.testRun + "&testRun=" + this.id.testRun + "&tierId=" + data.Tierid + "&serverId=" + data.Serverid + "&appId=" + data.Appid + "&stsignature=" + data.STSignature + "&depth=0" + "&strStartTime=" + this.id.startTime + "&strEndTime=" + this.id.endTime + "&minDepth=" + data.MinDepth + "&maxDepth=" + data.MaxDepth;  
    }
    else{
      second_url+= '/' + this.id.product.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/ASStackTraceData?testRun=" + this.id.testRun + "&tierId=" + data.Tierid + "&serverId=" + data.Serverid + "&appId=" + data.Appid + "&stsignature=" + data.STSignature + "&depth=0" + "&strStartTime=" + this.id.startTime + "&strEndTime=" + this.id.endTime + "&minDepth=" + data.MinDepth + "&maxDepth=" + data.MaxDepth;
    }
  ...
  getHotspotData() {
    ...
    if(this.commonService.enableQueryCaching == 1){
      endpoint_url += '/' + this.id.product.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/ASGroupFrameData?cacheId="+ this.id.testRun + "&testRun=" + this.id.testRun + "&tierName=" + this.id.tierName + "&serverName=" + this.id.serverName + "&appName=" + this.id.appName + "&strStartTime=" + this.id.startTime + "&strEndTime=" + this.id.endTime + "&depth=0";
    }
    else{
      endpoint_url += '/' + this.id.product.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/ASGroupFrameData?testRun=" + this.id.testRun + "&tierName=" + this.id.tierName + "&serverName=" + this.id.serverName + "&appName=" + this.id.appName + "&strStartTime=" + this.id.startTime + "&strEndTime=" + this.id.endTime + "&depth=0";
    }
...
endpoint_url += "&showCount=false";
  ...
  //Assign hotspot response value to the secondtarray
  ...
  downloadReport(downloadType: string) {
    let renameArray = { "DisplayName": "StackTrace Signature", "TopMethods": "StackTrace(Top Methods)", "MinDepth": "Min Depth", "MaxDepth": "Max Depth", "Count": "Thread Count(s)", "Min": "Min Duration", "Max": "Max Duration", "Average": "Avg Duration", "Variance": "Variance" };
    ...
    let colOrder = ["StackTrace Signature", "StackTrace(Top Methods)", "Min Depth", "Max Depth", "Thread Count(s)", "Min Duration", "Max Duration", "Avg Duration", "Variance"];
    ...
    let downloadObj: Object = {
      ...
      strSrcFileName: "allFrameASReport",
    ...
    let downloadFileUrl =  decodeURIComponent(this.getHostUrl(true)) + "/" +this.id.product.replace("/","") +"/v1/cavisson/netdiagnostics/ddr/downloadAngularReport";
