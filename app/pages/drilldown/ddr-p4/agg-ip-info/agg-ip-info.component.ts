...
export class AggIpInfoComponent implements OnInit,OnChanges {
  ...
  constructor(public commonService: CommonServices,
    ...
    private _navService: CavTopPanelNavigationService,private sessionService: SessionService) { }
  ...
  initializeColsForAggreate()
  {
    ...
    this.colsForAGGIP=[
        { field: 'renamebackendIPname', header: 'Integration Point', sortable: true, action: true, align: 'left', width: '60' },
        { field: 'actualbackendIPname', header: 'Discovered IPName', sortable: true, action: true, align: 'left', width: '60' },
        ...
        { field: 'mincount', header: 'Min count', sortable: 'custom', action: true, align: 'right', width: '45' },
        { field: 'maxcount', header: 'Max count', sortable: 'custom', action: true, align: 'right', width: '45' },
        { field: 'errorCount', header: 'Error(s)', sortable: 'custom', action: true, align: 'right', width: '40' },
        { field: 'avgNWDelay', header: 'Average Network Delay(ms)', sortable: 'custom', action: true, align: 'right', width: '50' },
        { field: 'totalNWDelay', header: 'Total Network Delay(ms)', sortable: 'custom', action: true, align: 'right', width: '50' },
  ...
  getHostUrl(isDownloadCase?): string {
    ...
    // if (!isDownloadCase && sessionStorage.getItem("isMultiDCMode") == "true" && this._cavConfigService.getActiveDC() == 'ALL') {
      ...
      // this.urlParam.testRun = this.testRun;
  ...
  setTestRunInHeader() {
    if (decodeURIComponent(this.urlParam['ipWithProd']).indexOf('netstorm') !== -1) {
      this.strTitle = 'Netstorm - BT IP Summary - Test Run : ' + this.testRun;
    } else {
      this.strTitle = 'Netdiagnostics Enterprise - BT IP Summary- Session : ' + this.testRun;
    }
  }
