...
@Component({
  selector: 'app-compare-hotspot',
  templateUrl: './compare-hotspot.component.html',
  styleUrls: ['./compare-hotspot.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class CompareHotspotComponent {
    ...
    isEnabledColumnFilterForIP = true
  ...
  productName: string;
    ...
    constructor(private commonService: CommonServices, private location: Location,
      ...
      private ddrRequest:DDRRequestService, private sessionService: SessionService ) { 
      ...
      console.log("ProductName ===> Constructor ==>", this.productName);
      }
    ...
    updateIdParam() {
      let endTime = 0;
      if (undefined != this.compareFPInfo.responseTime) {
        if (this.compareFPInfo.responseTime.toString().includes(','))
          endTime = Number(this.compareFPInfo.startTimeInMs) + Number(this.compareFPInfo.responseTime.toString().replace(/,/g, ""));
        else if (this.compareFPInfo.responseTime == '< 1')
          endTime = Number(this.compareFPInfo.startTimeInMs) + Number(0);
        else
          endTime = Number(this.compareFPInfo.startTimeInMs) + Number(this.compareFPInfo.responseTime);
      }
    ...
    fillData() {
      ...
      this.colsForHotspot = [
        ...
        { field: 'tierName', header: 'Tier', sortable: true, action: true, align: 'left', color: 'black', width: '50' },
      ...
      this.columns = [
        ...
        { field: 'threadname', header: 'Thread Name', sortable: true, action: true, align: 'left', color: 'black', width: '277' },
        { field: 'tierName', header: 'Tier', sortable: true, action: true, align: 'left', color: 'black', width: '50' },
        { field: 'serverName', header: 'Server', sortable: true, action: true, align: 'left', color: 'black', width: '75' },
        { field: 'appName', header: 'Instance', sortable: 'true', action: true, align: 'left', color: 'black', width: '75' },
        { field: 'hotspotstarttimestamp', header: 'HotSpot Entry Time', sortable: true, action: true, align: 'right', color: 'black', width: '130' },
        { field: 'hotspotduration', header: 'HotSpot Duration(Sec)', sortable: 'custom', action: true, align: 'right', color: 'blue', width: '116' },
        { field: 'totalDuration', header: 'Total Hotspot Duration(Sec)', sortable: 'custom', action: true, align: 'right', color: 'black', width: '116' },
        ...
        { field: 'threadpriority', header: 'Thread Priority', sortable: 'custom', action: true, align: 'right', color: 'black', width: '78' },
        { field: 'threadstackdepth', header: 'Stack Depth', sortable: 'custom', action: true, align: 'right', color: 'black', width: '78' },
        { field: 'FlowPathInstance', header: 'FlowpathInstance', sortable: 'true', action: true, align: 'left', color: 'black', width: '108' }
      ];
      ...
      this.colsForIP = [
        { field: "action", header: "Action", toolTip: "Action Links", width: 28, action: true, align: 'left' },
    ...
    getHostUrl(isDownloadCase?): string {
      ...
      if(!isDownloadCase && sessionStorage.getItem("isMultiDCMode")=="true" && this._cavConfigService.getActiveDC() == 'ALL')
      {
        ...
        console.log("all case url==>",hostDcName,"all case test run==>",this.testRun);
      }
    ...
    setTestRunInHeader() {
      if (decodeURIComponent(this.id.ipWithProd).indexOf("netstorm") != -1)
        this.strTitle = "Netstorm - HotSpots - Test Run : " + this.testRun;
      else
        this.strTitle = "Netdiagnostics Enterprise - HotSpots - Session : " + this.testRun;
    }
    ...
    getHotspotData() {
      ...
      if(this.commonService.enableQueryCaching == 1){
        url += '/' + this.productName.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/hotspotDataEX?cacheId="+ this.testRun + "&testRun=" + this.testRun ;
      }
      else{
        url += '/' + this.productName.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/hotspotDataEX?testRun=" + this.testRun ;
      }
      ...
      if (this.commonService.isFilterFromSideBar && Object.keys(this.commonService.hotspotFilters).length != 0) // sidebar filters to hotspot 
      {
        ...
        if (this.commonService.isValidParamInObj(hsParam, 'pubicIP') && this.commonService.isValidParamInObj(hsParam, 'publicPort') && this.commonService.isValidParamInObj(hsParam, 'ndeTestRun')) {
          if(this.commonService.enableQueryCaching == 1){
            url = hsParam['ndeProtocol'] + "://" + hsParam['pubicIP'] + ":" + hsParam['publicPort'] + '/' + this.productName.replace("/","") + '/v1/cavisson/netdiagnostics/ddr/hotspotDataEX?cacheId='+ hsParam['ndeTestRun'] + '&testRun=' + hsParam['ndeTestRun'];
          }
          else{
            url = hsParam['ndeProtocol'] + "://" + hsParam['pubicIP'] + ":" + hsParam['publicPort'] + '/' + this.productName.replace("/","") + '/v1/cavisson/netdiagnostics/ddr/hotspotDataEX?testRun=' + hsParam['ndeTestRun'];
          }
      ...
      else if (rowData == undefined) // ED TO DDR CASE - ROWDATA WILL BE UNDEFINED
      {
        ...
        console.log("END TIME=============================> WMS ==>", this.id.strEndTime);
        ...
        if (this.columnData == undefined) {
          console.log('m i here ======> ', this.compareFPInfo);
          urlParam += '&threadId=' + this.compareFPInfo['threadId'] +
            '&btCategory=' + this.compareFPInfo['catagoryName'] +
            // '&btCategory=Very%20Slow' +
            '&urlName=' + this.compareFPInfo['businessTransaction'] +
            '&urlIndex=' + this.compareFPInfo['urlIndex'] +
            '&flowpathInstance=' + this.compareFPInfo.flowpathInstance;
            console.log("FLOWPATH INST ======================> []===>", this.compareFPInfo['flowpathInstance']);
            console.log("FLOWPATH INST ======================> ", this.compareFPInfo.flowpathInstance);
        } else {
    ...
    if(instanceType) {
      ...
      if (instanceType.toLowerCase()  !== "dotnet") { 
        ...
        // url = this.getHostUrl() + '/' + this.productName.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/hotspotDataEX?testRun=1250&tierId=2&appId=2&serverId=1&appName=JVM14&serverName=10.10.50.14&tierName=tier_14&strStartTime=1571669865000&strEndTime=1571669941760&threadId=51&btCategory=Very%20Slow&urlName=/console/console.portal&urlIndex=1073741825&flowpathInstance=4611873638594863121"
        return this.ddrRequest.getDataUsingGet(url).subscribe(
          data => { this.doAssignValueHotspot(data) },
          error => {
            this.loading = false;
    ...
    getDCData() {
      let url = this.getHostUrl() + '/' + this.productName.replace("/","") + '/v1/cavisson/netdiagnostics/ddr/multiDC?testRun=' + this.testRun;
    ...
    getTierNamesForDC(dcName) {
      try {
        return new Promise((resolve, reject) => {
          ...
          this.getTieridforTierName(this.id.tierName).then(() =>{ 
          // resolve()
        });
        });
      } catch (e) {
    ...
    getTieridforTierName(tierName) {
      return new Promise((resolve, reject) => {
        try {
          ...
          url += '/' + this.productName.replace("/","") + "/analyze/drill_down_queries/NDAjaxController.jsp?strOperName=getTSAname&testRun=" + this.testRun + "&tierName=" + tierName;
    ...
    passDataForStackTrace(data: any) {
      ...
      console.log("stacktrace filter----"+ this.stackTraceFilter); 
       ...
       if (this.commonService.host == undefined || this.commonService.host == '' || this.commonService.host == null) {
        ...
        // }
        // else {
          second_url = decodeURIComponent(this.getHostUrl() + '/' + this.productName.replace("/",""));
          IP_url = decodeURIComponent(this.getHostUrl() + '/' + this.productName.replace("/",""));
       // }
      }
      else {
        this.dcProtocol = this.commonService.protocol;
        if (this.commonService.protocol && this.commonService.protocol.lastIndexOf("://") != -1) {
          second_url = this.commonService.protocol + this.commonService.host + ':' + this.commonService.port + '/' + this.productName.replace("/","");
          IP_url = this.commonService.protocol + this.commonService.host + ':' + this.commonService.port + '/' + this.productName.replace("/","");
          this.dcProtocol = this.commonService.protocol;
        } else if (this.commonService.protocol && this.commonService.protocol.length > 3) {
          second_url = this.commonService.protocol + "://" + this.commonService.host + ':' + this.commonService.port + '/' + this.productName.replace("/","");
          IP_url = this.commonService.protocol + "://" + this.commonService.host + ':' + this.commonService.port + '/' + this.productName.replace("/","");
          ...
          second_url = location.protocol + "//" + this.commonService.host + ':' + this.commonService.port + '/' + this.productName.replace("/","");
          IP_url = location.protocol + "//" + this.commonService.host + ':' + this.commonService.port + '/' + this.productName.replace("/","");
          this.dcProtocol = location.protocol;
        }
      }
      ...
      if(this.commonService.enableQueryCaching == 1){
        second_url += "/v1/cavisson/netdiagnostics/ddr/ASStackTraceData?cacheId="+ this.testRun + "&testRun=" + this.testRun + "&tierId=" + data.tierid + "&serverId=" +
      ...
      IP_url += "/v1/cavisson/netdiagnostics/ddr/backendNameForHotspot?cacheId="+ this.testRun + "&testRun=" + this.testRun + "&tierId=" + data.tierid + "&serverId=" + data.serverid + "&appId=" + data.appid + "&startTime=" + data.hotspotstarttimestamp + "&hsDuration=" + data.hsDurationInMs + "&strStartTime=" + this.id.startTime + "&strEndTime=" + this.id.endTime  ;
      }
      else{
        second_url += "/v1/cavisson/netdiagnostics/ddr/ASStackTraceData?testRun=" + this.testRun + "&tierId=" + data.tierid + "&serverId=" +
          data.serverid + "&appId=" + data.appid + "&threadId=" + data.threadid + "&hsTimeInMs=" + data.hsTimeInMs +
          "&hotSpotDuration=" + data.hsDurationInMs + '&instanceType=' + data.instanceType ;
       ...
       // this.getHotspotSecData(second_url); // Pass the URL to get Response from Rest service
        ...
        IP_url += "/v1/cavisson/netdiagnostics/ddr/backendNameForHotspot?testRun=" + this.testRun + "&tierId=" + data.tierid + "&serverId=" + data.serverid + "&appId=" + data.appid + "&startTime=" + data.hotspotstarttimestamp + "&hsDuration=" + data.hsDurationInMs + "&strStartTime=" + this.id.startTime + "&strEndTime=" + this.id.endTime  ;
      }
          //if(this._ddrData.splitViewFlag){
    ...
    */
    ...
    mergeAllStackTrace() {
      ...
      if (sessionStorage.getItem("isMerged") == "true") {
        ...
        if (this.commonService.host != undefined && this.commonService.host != '' && this.commonService.host != null) {
          ...
          // else      
          mergeStackUrl = decodeURIComponent(this.getHostUrl() + '/' + this.productName.replace("/",""));
        }
    ...
    //  this.hotSpotSecInfo.forEach((val, index) => {
     ...
     // });
      ...
      let integrationPointCallRenameArray = { "renamebackendIPname": "IP Name", "actualbackendIPname": "Discovered IP Name", "backendType": "Type", "backendStartTime": "Start Time"
      ...
      if (this.commonService.host != undefined && this.commonService.host != '' && this.commonService.host != null) {
        if(this.commonService.protocol && this.commonService.protocol.endsWith("://"))
        downloadFileUrl = this.commonService.protocol + this.commonService.host + ':' + this.commonService.port + '/' + this.productName.replace("/","");
        else
        downloadFileUrl = this.commonService.protocol + "://" + this.commonService.host + ':' + this.commonService.port + '/' + this.productName.replace("/","");
      } else {
        downloadFileUrl = decodeURIComponent(this.getHostUrl(true) + '/' + this.productName.replace("/",""));
    }
      ...
      if (this.visibleColsForHotspot.length !== 0) {
        for (let i = 0; i < this.columns.length; i++) {
          for (let j = 0; j < this.visibleColsForHotspot.length; j++) {
            if (this.columns[i].field === this.visibleColsForHotspot[j]) {
      ...
      if (this.visibleColsForIP.length !== 0) {
        for (let i = 0; i < this.cols.length; i++) {
          for (let j = 0; j < this.visibleColsForIP.length; j++) {
            if (this.cols[i].field === this.visibleColsForIP[j]) {
