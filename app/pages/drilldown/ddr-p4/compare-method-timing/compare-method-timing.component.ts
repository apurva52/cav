...
// import { CavConfigService } from "../../tools/actions/dumps/service/";
// import { CavTopPanelNavigationService } from "../service/cav-top-panel-navigation.service";
...
// import { MessageService } from '../service/ddr-message.service';
...
@Component({
  selector: 'app-compare-method-timing',
  templateUrl: './compare-method-timing.component.html',
  styleUrls: ['./compare-method-timing.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [EllipsisPipe],
})
export class CompareMethodTimingComponent {
  highcharts = Highcharts;
@Input() compareFPInfo:any[];
 ...
 filterCriteriaStr :string="";
  ...
  constructor(private commonServices:CommonServices,
    ...
    private sessionService: SessionService) {
      console.log("data ====> ", this.compareFPInfo);
     this.colour=['green','blue'];
      ...
      // this.compareFPInfo = this.commonServices.compare_Data;
      this.compareFPInfo = this.flowpathService.compareFlowpathData;
      console.log("IN METHOD TIMING COMPONENT", this.compareFPInfo);
  ...
  getComparedDBData()
  {
   // alert("in this case");
    ...
    let com_MT_url ='';
    let testRun;
    console.log("Compare Data =====> ",this.compareFPInfo);
    console.log("globalQueryParams",this.globalQueryParams);
        for(let i=0;i<this.compareFPInfo.length;i++)
        {
         // alert("startTimeInMS"+Number(this.compareFPInfo[i]['startTimeInMs'])+"fpduartion"+Number(this.compareFPInfo[i]['fpDuration']));
          let fpDuration=0;
          console.log("Compare data ========> ", this.compareFPInfo);
          if(this.compareFPInfo[i].responseTime == '< 1') 
            fpDuration = 0;
	        else if(this.compareFPInfo[i].responseTime.toString().includes(','))
            fpDuration =this.compareFPInfo[i].responseTime.toString().replace(/,/g,"");
        	else
	        	fpDuration = this.compareFPInfo[i].responseTime;
          ...
          cmdArgs+=this.testRun+","+this.compareFPInfo[i]['startTimeInMs']+","+endTime+","+this.compareFPInfo[i]['urlIndex']+","+this.compareFPInfo[i]['tierId']+","+this.compareFPInfo[i]['serverId']+","+this.compareFPInfo[i]['appId']+","+this.compareFPInfo[i]['flowpathInstance']+",%%%,";
        }
        ...
        if (this.commonServices.host != undefined && this.commonServices.host != '' && this.commonServices.host != null) {
          ...
          url = this.getHostUrl();
          console.log("Getting host Url ", url);
        }
        if(this.commonServices.enableQueryCaching == 1){
          ...
          com_MT_url=url+ '/' + this.productName.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/compareFPMethods?cacheId="+ testRun + "&cmdArgs="+cmdArgs;
        }
        else{
          com_MT_url=url+ '/' + this.productName.replace("/","") + "/v1/cavisson/netdiagnostics/ddr/compareFPMethods?cmdArgs="+cmdArgs;
        }
  ...
  downloadReports(reports: string) {
   ...
   setTestRunInHeader() {
    if (decodeURIComponent(this.globalQueryParams.ipWithProd).indexOf('netstorm') !== -1) {
      this.strTitle = 'Netstorm - Compared MethodTiming - Test Run : ' + this.testRun;
    } else {
      this.strTitle = 'Netdiagnostics Enterprise - Compared MethodTiming - Session : ' + this.testRun;
    }
  }
