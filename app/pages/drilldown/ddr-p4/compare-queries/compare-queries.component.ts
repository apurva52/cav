...
@Component({
  selector: 'app-compare-queries',
  templateUrl: './compare-queries.component.html',
  styleUrls: ['./compare-queries.component.css']
})

export class CompareQueriesComponent implements OnInit,OnChanges {
...
errMsg: Message[];
  ...
  {
    this.loading=true;
    console.log('this.compareFPInfo======DB Query===>',this.compareFPInfo);
  ...
  getComparedDBData()
  {
   // alert("in this case");
    ...
    console.log("globalQueryParams",this.globalQueryParams);
        for(let i=0;i<this.compareFPInfo.length;i++)
        {
         // alert("startTimeInMS"+Number(this.compareFPInfo[i]['startTimeInMs'])+"fpduartion"+Number(this.compareFPInfo[i]['fpDuration']));
          ...
          cmdArgs = '1250,1571669379000,1571669379887,1073741828,2,1,2,4611873638470533378,%%%,1250,1571669470000,1571669470858,1073741831,2,1,2,4611873638493894923,%%%,'
    ...
    if(this.commonServices.enableQueryCaching == 1){
       com_db_url=url + '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/compareFPQueries?cacheId="+ testRun + "&cmdArgs="+cmdArgs ;
    }
    else{
       com_db_url=url + '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/compareFPQueries?cmdArgs="+cmdArgs;
    }
...
showFilterCriteria() {
  ...
  if (this.compareFPInfo[0]['urlName'] != "NA" && this.compareFPInfo[0]['urlName'] != "" && this.compareFPInfo[0]['urlName'] != undefined && this.compareFPInfo[0]['urlName'] != "undefined" && this.compareFPInfo[1]['urlName'] != "NA" && this.compareFPInfo[1]['urlName'] != "" && this.compareFPInfo[1]['urlName'] != undefined && this.compareFPInfo[1]['urlName'] != "undefined")
  {
   ...
   setTestRunInHeader() {
    if (decodeURIComponent(this._ddrData.getHostUrl()+'/'+this.productName).indexOf('netstorm') !== -1) {
      this.strTitle = 'Netstorm - Compared DBRequest - Test Run : ' + this.testRun;
    } else {
      this.strTitle = 'Netdiagnostics Enterprise - Compared DBRequest - Session : ' + this.testRun;
    }
  }
