...
@Component({
  selector: 'app-drilldown-db-report',
  templateUrl: './drilldown-db-report.component.html',
  styleUrls: ['./drilldown-db-report.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [EllipsisPipe]
})
export class DrillDownDbReportComponent implements OnInit {
...
this.loading = true;
      ...
      if ((this._router.url.indexOf('?') != -1) && (this._router.url.indexOf('/home/ED-ddr/dbReport') != -1)) {
          ...
          console.log("inside db this.commonService.dbDataFromFPInED--",this.commonService.dbDataFromFPInED)
...
console.log('This.id after ED_ddr', this.id, 'ajaxtimeout', this.commonService.ajaxTimeOut);
      this.commonService.removeFromStorage();
  ...
  createColumns() {
      this.cols = [
          ...
          { field: 'sqlQuery', header: 'Query', sortable: true, action: true, align: 'left', color: 'black', width: '150' },
  ...
  getQueryData() {
      ...
      if (this.commonService.host == undefined || this.commonService.host == '' || this.commonService.host == null) {
        ...
        if(this.commonService.enableQueryCaching == 1){  
              url += '/' + this.productName.replace("/", "") + "/v1/cavisson/netdiagnostics/ddr/flowpathqueryEx?cacheId="+ this.testRun + "&testRun=" + this.testRun;
        }else{
              url += '/' + this.productName.replace("/", "") + "/v1/cavisson/netdiagnostics/ddr/flowpathqueryEx?testRun=" + this.testRun;
        }
      ...
      console.log(" this.commonService.dbFilters['strOrderBy']--", this.commonService.dbFilters['strOrderBy']);
          ...
          if (this.commonService.isValidParamInObj(dbParam, 'ndeProtocol') && this.commonService.isValidParamInObj(dbParam, 'pubicIP') && this.commonService.isValidParamInObj(dbParam, 'publicPort') && this.commonService.isValidParamInObj(dbParam, 'ndeTestRun')) {
              if(this.commonService.enableQueryCaching == 1){
                  url = dbParam['ndeProtocol'] + "://" + dbParam['pubicIP'] + ":" + dbParam['publicPort'] + '/' + this.productName.replace("/", "") + '/v1/cavisson/netdiagnostics/ddr/flowpathqueryEx?cacheId='+ dbParam['ndeTestRun'] + '&testRun=' + dbParam['ndeTestRun'];    
              }
           else{
                  url = dbParam['ndeProtocol'] + "://" + dbParam['pubicIP'] + ":" + dbParam['publicPort'] + '/' + this.productName.replace("/", "") + '/v1/cavisson/netdiagnostics/ddr/flowpathqueryEx?testRun=' + dbParam['ndeTestRun'];
              }
...
// }
  ...
  getDbFlowpathData(data) {
      ...
      if (this._router.url.indexOf('/home/ddrCopyLink/') != -1) {
          ...
          this._router.navigate(['/drilldown/flow-path']);
      }
  }
  ...
  getTierNamesForDC(dcName) {
      try {
          return new Promise((resolve, reject) => {
              ...
              this.getTieridforTierName(this.tierName).then(() => { console.log("******then"); 
              // resolve() 
            });
          });
      } catch (e) {
...
getNDEInfo(res) {
  ...
  getTieridforTierName(tierName) {
      return new Promise((resolve, reject) => {
          try {
              ...
              url += '/' + this.id.product.replace("/", "") + "/analyze/drill_down_queries/NDAjaxController.jsp?strOperName=getTSAname&testRun=" + this.testRun + "&tierName=" + tierName;
  ...
  setReportHeader() {
      if (decodeURIComponent(this.id.ipWithProd).indexOf("netstorm") != -1)
          this.strTitle = "Netstorm - DB Queries Report - Test Run : " + this.testRun;
      else
          this.strTitle = "Netdiagnostics Enterprise - DB Queries Report - Session : " + this.testRun;
  }
