...
// import { DdrBreadcrumbService } from '../../../pages/tools/actions/dumps/service/';
...
@Component({
  ...
  styleUrls: ['./flowpath-group-by.component.scss']
})
...
if(this.stateID){
  ...
  for (const timeInfo of state.time) {
    ...
    this.breadcrumb.add({label: 'Flowpaths Group By BT', routerLink: '/drilldown/flowpath-group-by',  queryParams: { state: this.stateID}});
    ...
    this.sideBarFlowpathGroupBy = this.globalDrillDownFilterService.sideBarUIObservable$.subscribe((temp) => {
      if(this.globalDrillDownFilterService.currentReport == "FPG_BT")
      {   
     // console.log('temp============',temp);
      ...
      this.loading = true;
      this.commonService.fpGroupByFilters = temp;
      this.fillData();
      }
...
testRun: string;
  ...
  constructor(public commonService: CommonServices,
    ...
    private ddrRequest:DDRRequestService,private route: ActivatedRoute,
  ...
  breadcrumb: BreadcrumbService ) {
    ...
    createDynamicColumn() {
      ...
      console.log('fpParam====createDynamicColumn====>',fpParam)
      this.cols = [
      ...
      this.column = [
        { field: 'urlName', header: 'Business Transaction', sortable: true, action: true, align: 'left', width: '100' },
        { field: 'fpCount', header: 'Flowpath Count', sortable: 'custom', action: true, align: 'right', width: '100' }
      ];
 ...
 if(this.commonService.enableQueryCaching == 1) {
   url += '/' + this.productName + '/v1/cavisson/netdiagnostics/ddr/flowpathreport?cacheId='+ this.testRun + '&testRun=' + this.testRun ;
 }    
else{
  url += '/' + this.productName + '/v1/cavisson/netdiagnostics/ddr/flowpathreport?testRun=' + this.testRun ;
}
      ...
      { 
if(this.commonService.enableQueryCaching == 1){
  url =  this.getHostUrl() + '/' + this.productName + '/v1/cavisson/netdiagnostics/ddr/flowpathreport?cacheId='+ this.testRun + '&testRun=' + this.testRun ;
}
else {
  url =  this.getHostUrl() + '/' + this.productName + '/v1/cavisson/netdiagnostics/ddr/flowpathreport?testRun=' + this.testRun ;
}
      ...
      if (this.visibleCols.length !== 0) {
        for (let i = 0; i < this.column.length; i++) {
          for (let j = 0; j < this.visibleCols.length; j++) {
            if (this.column[i].field === this.visibleCols[j]) {
              this.column[i].action = true;
              break;
            } else {
              this.column[i].action = false;
            }
          }
        }
      }
...
toggleFilters() {
  ...
  if (me.isEnabledColumnFilter === true) {
    me.filterTitle = 'Hide Column Filters';
  } else {
    me.filterTitle = 'Show Column Filters';
  }
}   
 ...
 return paginatorArr;
  ...
  //used to open Flowpath Report from Flowpath by BT report in LRM UI
  openFlowpath(RowData){
    const me = this;
    console.log("fpGroupbyRowData row data ---",RowData);
    me.sessionService.setSetting("fpGroupbyRowData",RowData);
    this._router.navigate(['/drilldown/flow-path'], { queryParams: {state: me.stateID}});
  } 
  ...
  setTestRunInHeader() {
    if (decodeURIComponent(this.urlParam.ipWithProd).indexOf('netstorm') !== -1) {
      this.strTitle = 'Netstorm - Flowpath Group By Business Transaction- Test Run : ' + this.testRun;
    } else {
      this.strTitle = 'Netdiagnostics Enterprise - Flowpath Group By Business Transaction- Session : ' + this.testRun;
    }
  }
   ...
   let downloadObj: Object = {
    ...
    let downloadFileUrl =  decodeURIComponent(this.getHostUrl(true) + '/' + this.productName) +
  ...
  randomNumber(){
    ...
    onCancelQuery(){
    let url = "";
    url = decodeURIComponent(this.getHostUrl() + '/' + this.productName) +"/v1/cavisson/netdiagnostics/webddr/cancleQuery?testRun="+ this.urlParam.testRun +"&queryId="+this.queryId;
