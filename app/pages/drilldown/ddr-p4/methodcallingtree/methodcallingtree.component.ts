import { Component, OnInit, OnDestroy, OnChanges, ChangeDetectorRef,SimpleChange, Renderer2, ViewChild, ElementRef, Input  ,ViewChildren,QueryList,AfterViewInit } from '@angular/core';
import { TreeNode, Message, MenuItem } from 'primeng/api';
...
import { SelectItem, TreeTable } from 'primeng';
  ...
  testRun: string;
   ...
   constructor(
    ...
    private sessionService:  SessionService
      ...
      this.testRun = this.sessionService.session.testRun.id;
    ...
    if (this.columnData != undefined) {
      ...
      if (this.queryParams['fpDuration'] != undefined) {
        if (this.queryParams['fpDuration'] == '< 1') {
          this.queryParams.responseTime = 0;
          this.fpDuration = 0
        }
        else if (this.queryParams['fpDuration'].toString().includes(',')) {
          this.queryParams.responseTime = this.queryParams.responseTime.toString().replace(/,/g, "");
          this.fpDuration = this.queryParams.responseTime.toString().replace(/,/g, "");
        } else {
          this.fpDuration = this.queryParams.responseTime;
        }
      }
      this.trStartTime = this.queryParams.startTimeInMs;
      this.trEndTime = Number(this.queryParams.startTimeInMs) + Number(this.queryParams.responseTime);
    } else {
 ...
 this.items = [
  ...
  assignMCTColumns()
  {
    ...
    this.colsForMCT = [
      ...
      { field: 'dynLogFlag', header: 'Flag', sortable: false ,action: true, align: 'left', width: '40px'},
      { field: 'api', header: 'API', sortable: false ,action: false, align: 'left', width: '40px'},
      { field: 'timeStamp', header: 'ElapsedTime(ms)', sortable: false, action: true, align: 'right', width: '40px'}];
    
    this.colsForMCT.push({ field: 'wallTime', header: 'WallTime(ms)', sortable: false, action: true, align: 'right', width: '40px'});
     this.colsForMCT.push(  { field: 'percentage', header: 'Percentage', sortable: false, action: true, align: 'right', width: '40px'});
      this.colsForMCT.push( { field: 'cpuTime', header: 'CPUTime(ms)', sortable: false, action: true, align: 'right', width: '40px'});
        if(this.isEnabledQAS)
      {
     this.colsForMCT.push(  { field: 'queueTime', header: 'ThreadQueueTime(ms)', sortable: false, action: true, align: 'right', width: '35px'});
     this.colsForMCT.push({ field: 'selfTime', header: 'SelfTime(ms)', sortable: false, action: true, align: 'right', width: '40px'});
      }
      ...
      this.colsForMCT.push( { field: 'ioTime', header: 'IOTime(ms)', sortable: false, action: false, align: 'right', width: '40px'});
       ...
       this.colsForMCT.push({ field: 'threadId', header: 'ThreadId', sortable: false, action: true, align: 'right', width: '40px'});
      this.colsForMCT.push( { field: 'threadName', header: 'Thread', sortable: false, action: true, align: 'left', width: '40px'});
      this.colsForMCT.push({ field: 'methodArgument', header: 'Arguments', sortable: false, action: true, align: 'left', width: '40px'});
       this.colsForMCT.push({ field: 'instanceType', header: 'InstanceType', sortable: false, action: false, align: 'left', width: '40px'});
      this.colsForMCT.push( { field: 'appName', header: 'Instance', sortable: false, action: true, align: 'left', width: '40px'});
  ...
  getMCTDataFunc() {  
   // console.log(this.queryParams);
    this.trStartTime = this.queryParams['startTimeInMs'];
    if (this.queryParams['responseTime'] != undefined) {
      if (this.queryParams['responseTime'] == '< 1')
        this.fpDuration = 0;
      else if (this.queryParams['responseTime'].toString().includes(','))
        this.fpDuration = this.queryParams.responseTime.toString().replace(/,/g, "");
      else
        this.fpDuration = this.queryParams.responseTime;
    }
    ...
    this.menuIte = [
      { label: 'Method Details', icon: 'fa fa-info-circle',command: (event) => this.viewMethodDetail(this.selectedNode) },
    ];
    this.menuIteCallOut = [
      { label: 'Method Details', icon: 'fa fa-info-circle',command: (event) => this.viewMethodDetail(this.selectedNode) },
    ];
    ...
    if(this._ddrData.enableViewSourceCode !== 1 || this._ddrData.enableViewSourceCode !== 0) {
      ...
      this.menuIte.push(
        { label: 'View Source Code', icon: 'fa fa-info-circle',command: (event) => this.viewSourceCode(this.selectedNode) }
      );
    ...
    this.getMCTData().subscribe(res => {
      let mctData = <any> res;
      console.log('mctData==getMCTData=>',mctData)
      this.loader = false;
      this.loading = false;
...
let url = this.getHostUrl();
    url += '/'  + this.productName + '/v1/cavisson/netdiagnostics/webddr/enableViewSourceCode';
  ...
  if(this._ddrData.enableViewSourceCode ===1 ){
    this.menuIte.push(
      { label: 'View Source Code', icon: 'fa fa-info-circle',command: (event) => this.commonModuleCall(this.selectedNode) }
    );
    this.menuIteMethod.push(
      { label: 'View Source Code', icon: 'fa fa-info-circle',command: (event) => this.commonModuleCall(this.selectedNode) }
    );
 ...
 // variableselection: string;
  ...
  commonModuleCall(rowData){
    ...
    this.argsForDynamicLogging =[this.dynTierName,this.dynServerName,this.dynAppName,this.dynFqm,this.productName,this.testRun,this.dynEntrySeqNum,this.dynFpInstance,"0"]
  ...
  viewSourceCode(node) {
    ...
    console.log("The value inisde the tier is ..",this.testRun);
    ...
    this.getServerApp();
    let url = this.getHostUrl();
    url += '/'  + this.productName + '/v1/cavisson/netdiagnostics/ddr/readSourceCode?&fqm=' +  fqmtemp + "&tierName=" + this.dynTierName +"&serverName=" + this.dynServerName + "&instanceName=" + this.dynAppName;
  ...
  assignValue(dataVal: any, nodeInfo: any) {
   ...
   let url = this.getHostUrl() + '/'  + this.productName + '/v1/cavisson/netdiagnostics/webddr/actualTrackPoints?testRun=' + this.testRun + "&fpInstance=" + this.fpInstanceInfo[this.fpInstanceInfoKeys[0]][0] + "&methSeqNo="+entrySeqNumTemp ;
  ...
  jsondata_url += '/configUI/custom/dl/getTraceDetails/'+this.testRun+'/'+this.dynTierName+'/'+this.dynServerName+'/'+this.dynAppName+'?fqc='+this.dynFqc;
...
this.openEditDialog = false;
  ...
  getProfileData(){
    let url = this.getHostUrl();
    url += '/configUI/custom/dl/getProfileListAndAppliedProfile/'+this.testRun+'/'+this.dynTierName+'/'+this.dynServerName+'/'+this.dynAppName + '/instance';
  ...
  getServerApp(){
    let url = this.getHostUrl() + '/'  + this.productName + '/v1/cavisson/netdiagnostics/webddr/getServerInstance?testRun=' + this.testRun + "&tier=" + this.dynTierName;
...
//  }
  ...
  openDLbasedOnFQC(){
    ...
    this.argsForDynamicLogging =[this.dynTierName,this.dynServerName,this.dynAppName,this.dynFqm,this.productName,this.testRun,"",this.dynFpInstance,"1"]
    this.ShowCommonDLModule= true;
    ...
    if (node.children == undefined) {
      if (this.count == 0) {
        if ( node.data.methodArgument == "-" || node.data.methodArgument.includes("Method Arguments")) {
          if (node.data.methodArgument == "-") {
            node.data.methodArgument = this.queryParams.url
          } else {
...
expandSameLeveluptoFixLevel(node,limit,parentLineNumber,childArrLength)
{
 ...
 methodCount=Number(this.queryParams.methodsCount.toString().replace(/,/g, ""));
 if(this.commonService.enableQueryCaching == 1){
  url += '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/fpDetailsMCT?cacheId="+ this.testRun + "&filterGreaterThan="+this.txtFilterWallTime+"&filterTop="+this.txtFlterMethodLevel+"&packageList=&classList=&methodList=&testRun=" + this.testRun + "&flowPathInstance=" + this.queryParams.flowpathInstance + "&strStartTime=" + startTimetoPass + "&strEndTime=" + endTimetoPass + "&entryResponseTime=" + this.fpDuration + "&thresholdWallTime=" + this.txtThresholdWallTime + "&FromAngular=1&enableMergeCase="+this.enableMergeCase+"&ignoreFilterCallouts="+this.ignoreFilterCallOuts+"&methodCount="+methodCount+"&filterMethods=false&thresholdDiffElapsedTime="+this.txtThresholdelapsedTime;
 }else{
   url += '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/fpDetailsMCT?filterGreaterThan="+this.txtFilterWallTime+"&filterTop="+this.txtFlterMethodLevel+"&packageList=&classList=&methodList=&testRun=" + this.testRun + "&flowPathInstance=" + this.queryParams.flowpathInstance + "&strStartTime=" + startTimetoPass + "&strEndTime=" + endTimetoPass + "&entryResponseTime=" + this.fpDuration + "&thresholdWallTime=" + this.txtThresholdWallTime + "&FromAngular=1&enableMergeCase="+this.enableMergeCase+"&ignoreFilterCallouts="+this.ignoreFilterCallOuts+"&methodCount="+methodCount+"&filterMethods=false&thresholdDiffElapsedTime="+this.txtThresholdelapsedTime;
 }
    this.loading = false;
  ...
  //  }
    if(this.commonService.enableQueryCaching == 1){
      url += '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/methodTimingReport?cacheId="+ this.testRun + "&testRun=" + this.testRun + "&flowpathInstance=" + this.queryParams.flowpathInstance + "&strStartTime=" + startTimetoPass + "&strEndTime=" + endTimetoPass + "&tierId=" + this.queryParams.tierId + "&serverId=" + this.queryParams.serverId + "&appId=" + this.queryParams.appId + "&type=method&entity=0";
    }
    else{
      url += '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/methodTimingReport?testRun=" + this.testRun + "&flowpathInstance=" + this.queryParams.flowpathInstance + "&strStartTime=" + startTimetoPass + "&strEndTime=" + endTimetoPass + "&tierId=" + this.queryParams.tierId + "&serverId=" + this.queryParams.serverId + "&appId=" + this.queryParams.appId + "&type=method&entity=0";
    }
  ...
  openAutoInstDialog()
  {
      ...
      //  else
       url=this.getHostUrl();
        url += '/' + this.productName + '/v1/cavisson/netdiagnostics/ddr/getTestRunStatus?testRun='+this.testRun;
       console.log('url *** ', url);
       this.ddrRequest.getDataUsingGet(url).subscribe(resp => {
         let res = <any> resp;
	      ...
	      if(this.queryParams.Instance_Type.toLowerCase() == 'java')
              ...
              this.queryParams.serverName,this.queryParams.serverId,"-1",this.queryParams.urlName,"DDR",testRunStatus[0].status,this.testRun];           
           }
           else
           {
  ...
  openSeqDiagram() {
    ...
    urlforJsp += "/" + this.productName + "/integration.jsp?testRun=" + this.testRun + "&urlname=" + this.queryParams.urlName
  ...
  //   }
    ...
    if (this.queryParams.responseTime != undefined && this.queryParams.responseTime != '' && this.queryParams.responseTime != 'NA')
      this.filterCriteria += ", ResponseTime=" + this.formatter(this.queryParams.responseTime) + "(ms)";
  ...
  setTestRunInHeader() {
    if (decodeURIComponent(this.id.ipWithProd).indexOf("netstorm") != -1) {
      this.strTitle = "Netstorm - Method Summary Report - Test Run : " + this.testRun;
      this.strTitleMct = "Netstorm - Method Calling Tree - Test Run : " + this.testRun;
    }
    else {
      this.strTitle = "Netdiagnostics Enterprise - Method Summary Report - Session : " + this.testRun;
      this.strTitleMct = "Netdiagnostics Enterprise - Method Calling Tree - Session : " + this.testRun;
    }
  }
...
//    let colOrder1 = ["Methods", 'S.N.', "Arguments", "Thread", "ThreadId", "Tier", "Server", "Instance", "Class", "Package", "StartTime", "WallTime(ms)", "CPUTime(ms)", "Percentage", "PageName", "API", "WaitTime(ms)", "SyncTime(ms)", "IOTime(ms)", "SuspensionTime(ms)", "InstanceType", "Async_Call"];
      ...
      if (index == 0) {
        if (val['methodArgument'] == "-" || val['methodArgument'].includes("Method Arguments")) {
          if (val['methodArgument'] == "-") {
            val['methodArgument'] = this.queryParams.url
          } else {
            val['methodArgument'] = this.queryParams.url + '\n' + val['methodArgument'];
          }
        }
  ...
  //    delete val['timeStamp'];
    ...
    if (this.commonService.protocol && this.commonService.host != undefined && this.commonService.host != '' && this.commonService.host != null) {
      if (this.commonService.protocol && this.commonService.protocol.endsWith("://"))
        downloadFileUrl = this.commonService.protocol + this.commonService.host + ':' + this.commonService.port + '/' + this.productName;
      else if (this.commonService.protocol)
        downloadFileUrl = this.commonService.protocol + "://" + this.commonService.host + ':' + this.commonService.port + '/' + this.productName;
      else
        downloadFileUrl = "//" + this.commonService.host + ':' + this.commonService.port + '/' + this.productName;
    } else {
  ...
  donwloadReportsRepeatedMethod(reports:string)
  {
	 ...
	 delete val['dynLogFlag'];
      ...
      downloadFileUrl += '/'+ this.productName + '/v1/cavisson/netdiagnostics/ddr/downloadAngularReport';
      if (sessionStorage.getItem("isMultiDCMode") == "true" && (downloadFileUrl.includes("/tomcat")|| downloadFileUrl.includes("/node"))) {
        this.ddrRequest.getDataInStringUsingPost(downloadFileUrl, downloadObj).subscribe(res =>
          (this.openDownloadReports(res)));
      }
...
let arrayVal=[];
 ...
 //  console.log(val,">>>   val")
  ...
  openExpandedCallouts()
  {
   ...
   methodCount=Number(this.queryParams.methods.toString().replace(/,/g, ""));
    new_MCT_url += '/' + this.productName + "/v1/cavisson/netdiagnostics/webddr/fpDetailsMCT?filterGreaterThan="+this.txtFilterWallTime+"&filterTop="+this.txtFlterMethodLevel+"&packageList=&classList=&methodList=&testRun=" + this.testRun + "&flowPathInstance=" + this.queryParams.flowpathInstance + "&strStartTime=" + startTimetoPass + "&strEndTime=" + endTimetoPass + "&entryResponseTime=" + this.fpDuration + "&thresholdWallTime=" + this.txtThresholdWallTime + "&FromAngular=2&enableMergeCase="+this.enableMergeCase+"&ignoreFilterCallouts="+this.ignoreFilterCallOuts+"&methodCount="+methodCount+"&filterMethods="+this.filterMethods+"&thresholdDiffElapsedTime="+this.txtThresholdelapsedTime;
    this.loading = false;
 ...
 // console.log("parent seqn no vaalu-----", node,"----line number----------------",lineNumber)
  ...
  rowTrackBy(index: number, row: any) { return row.id; }
   ...
   // console.log("elemtn value",$event.element,"delta",$event.delta);
    ...
    this.colsForMCT.forEach((val,index)=>{ console.log("index value---",index,"value------",val);
