...
declare var showheatmapforpage;
...
openPageDump(currentpageindex:number,pageinstance:number, snapshotIndex:number, mode :string){
  ...
  (
    currentpageindex,this.selectedSession.startTime,this.selectedSession.sid,pageinstance,this.selectedSession.partitionID,snapshotIndex).subscribe(response =>{   
            ...
            this.pagedumpLoad = true;   
            this.pagedumploading = false; 
            // kept true to wait for heatmap data
            this.busy = true;
            //this.generateHeatMap(null);
          }); 
     });
}
...
generateHeatMap(dataObj)
{
  ...
  //http://10.20.0.53:8001/netvision/reports/heatmapdata.jsp?startDateTime=-180184385924&endDateTime=198247614076&currentPage=Home&strOperationName=heatmapforpage&devicetype=PC&channel=1&userSegment=0            
  let s = null; let e = null; let duration = null;
  ...
  let r = this;
  let reqlimit;
  if(this.nvconfigurations.heatLimit){
    reqlimit=this.nvconfigurations.heatLimit;
  }
  else{
   reqlimit =1000;
  }
  this.showDates = false;
  this.nvhttp.getHeatMapData(st,et,pagename,deviceID,channelid,usersegmentid,reqlimit).subscribe((state : Store.State) =>{
    if(state instanceof NVPreLoadingState){
        // show loader
        this.busy = true;
    }
    if(state instanceof NVPreLoadedState){
      console.log("data loaded in generateHeatMap : ", state.data);
      let response = state.data;
      console.log("response : ", response);
      if(response.length == 0)
      {
         alert("No data for the given timerange");
         //console.log('inside suggesttimefromlptable condiiton');
         this.suggesttimefromlptable(st,et,r.lptabledata);
         this.busy =false;
         return null;
      }
     //r.closePopup();
     //r.expandFrame();
     let obj = {"key" : "heat", "data" : {type:"Heat Map"}};
     r.replayHandler.broadcast('msg',obj);

     showheatmapforpage(this.replayIframe.nativeElement.contentWindow.document, response);
     this.hideIcon();
     this.busy = false;
    }
