...
import { CLICK_MAP_DATA } from './../click-map/service/click-map.dummy';
...
export class NavigationMapComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    me.data = VIDEO_DATA;
    console.log("Navigationmap  component 0 ");
    me.route.queryParams.subscribe(params=>{
      console.log("NavigationMap  component 1 ");
  ...
  initC(){
    const me = this;
    console.log("initC called in navigation map");
  ...
  addCanvas() 
  {
   ...
   findMatchingSibling(ctn)
   {
    ...
    createHeatMapNodes(tag,color,level,i,j,z,k,perc)
    {
      ...
      if(level == 2)
      {
      ih += '<li count="'+i+'" j="'+j+'" class="an'+i+'" pathid='+this.trimCSSPath(this.funnelData[i].children[j].ctn.id) +' style="margin-top:2px;display:none" ><span class="btn nvrbtn-'+color+'  btn-'+color+'" style="width:90%" onclick="showInformation('+i+','+j+')" onmouseover="showHeatMapChildElement('+i+','+j+',true)" onmouseout="showHeatMapChildElement('+i+','+j+',false)">';
      ...
      {
      ih += '<li  count="'+i+'" j="'+j+'"  pathid='+this.trimCSSPath(this.funnelData[i].ctn.id) +' style="margin-top:2px;display:block" ><span  class="btn nvrbtn-'+color+' btn-'+color+'" style="width:90%"  onclick="showInformation('+i+');showHeatMapElement('+i+' ,true)" >';
      ...
      ih += '<span title="'+tag+'" style="font: 12px Arial, Helvetica, sans-serif;color: white;">'+tag.substring(0,20)+'...</span>';
      if(k == "none")
      ih += '<span class="glyphicon glyphicon-plus" aria-hidden="true" width="15px" height="15px" style="float:right;cursor:pointer;" name = "'+i+'" onclick=showInnDiv("'+i+'")></span>';
      }
      else
      {
      ih += '<span title="'+tag+'" style="font: 12px Arial, Helvetica, sans-serif;color: white;">'+tag+'</span>';
