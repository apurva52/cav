...
import { HomePageTabLoadedState,HomePageTabLoadingState,HomePageTabLoadingErrorState } from './browser-version-service.state';
@Injectable({
  providedIn: 'root'
})
export class PageTabService extends Store.AbstractService {
    ...
    LoadPageTabData(filtercriteria) : Observable<Store.State>{
        ...
        me.controller.get(path,null,base).subscribe((data: any) =>{
            ...
            output.next(new HomePageTabLoadingErrorState(e));
            output.complete();
        });
        return output;
    }
