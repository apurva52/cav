...
import { PAGE_TABLE } from './service/page.dummy';
import { PageTable } from './service/page.model';
...
export class PageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log('me.datapagefilter : ', me.datapagefilter);
  ...
  loadPagePerformanceData(filtercriteria) {
    ...
    me.pageTabService.LoadPageTabData(filtercriteria, 'page').subscribe(
      (state: Store.State) => {
  ...
  private processxgraphData(xaxisnamearr) {
    ...
    for (let i = 0; i < xaxisnamearr.length; i++) {
      ...
      let obj = xaxisnamearr[i].pagename + "( " + count + " )";
      xaxisname.push(obj)
    }
    return xaxisname;
  }
  private processygraphData(yaxisdtarr) {
    ...
    series.push({ "name": "First Byte", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "Perceived Render ", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "DNS Time", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "Secure", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "Cache", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "Connection", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "Network", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "Exit PCT", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "First Paint", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "First Content Paint", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "Time to Interactive", "pointWidth": 15, "visible": false, "data": [] });
    series.push({ "name": "First Input Delay", "pointWidth": 15, "visible": false, "data": [] });
  ...
  private onLoaded(state: HomePageTabLoadedState) {
    ...
    PAGE_TABLE.charts[0]["highchart"]["chart"]["height"] = xaxis.length * seriesdata.length * 20;
    PAGE_TABLE.charts[0]["highchart"]["series"] = seriesdata as Highcharts.SeriesOptionsType[];
    PAGE_TABLE.charts[0]["highchart"]["xAxis"]["categories"] = xaxis;
    me.data.charts = PAGE_TABLE.charts;
    setTimeout(() => {
  ...
  showResponse(key, overviewobj) {
    ...
    if (key == 1) {
      ...
      this.isShowPerformancedetail = false;
