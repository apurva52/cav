...
export class ProcessResourceData {
  ...
  getFormattedData() {
    ...
    for (let i = 0; i < this.rdata.length; i++) {
      ...
      if (!obj.hasOwnProperty(contentType)) {
        ...
        obj[contentType]["count"] = 1;
        obj[contentType]["countpct"] = ((1 * 100) / this.rdata.length).toFixed(2);
        ...
        obj[contentType].count++;
        obj[contentType]["countpct"] = ((obj[contentType]["count"] * 100) / this.rdata.length).toFixed(2);
