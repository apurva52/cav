...
//  private breadcrumbService :DdrBreadcrumbService
  ...
  //  this.heapDumpView =true;
    ...
    this.HeapData = this.heapDumpService.$ibmAnalyzerData;
    ...
    this.ibmHeapPathUrl = this.heapDumpService.$ibmAnalyzerUrl;
  ...
  getHeaderInfoAndUrl(){
    if(this.ibmHeapPathUrl) {
      ...
      console.log("this._ddrData.ibmHeapFileName",this._ddrData.ibmHeapFileName);
      // var ibmFileName = this._ddrData.ibmHeapFileName;
      var ibmFileName = this.heapDumpService.$ibmHeapFileName;
      var uploadedFilePath = sessionStorage.getItem('workPath') + "/logs/heapDump";
      this.headerInfo += "filePath="+ uploadedFilePath+ ",fileName="+ibmFileName;
      var customUrl = decodeURIComponent(this._ddrData.getHostUrl(true) + '/' + this.productName);
      customUrl += '/v1/cavisson/netdiagnostics/ddr/analyzeHeap?testRun='+this.testRun+'&filePath='+ uploadedFilePath +'&fileName='+ibmFileName ;
      this.ibmHeapPathUrl = customUrl;
    ...
    downloadReports(reports: string) {
      ...
      let downloadObj: Object = {
        ...
        jsonData: JSON.stringify(downloadData)
          ...
          // }
          // else
              downloadFileUrl = decodeURIComponent(this._ddrData.getHostUrl(true) + '/' +  this.productName);
          // downloadFileUrl = this._ddrData.getHostUrl(true) + '/' +  this.productName;
      ...
      openDownloadReports(res) {
          ...
          let downloadFileUrl = this._ddrData.getHostUrl();
