...
@Input('value') threadDumpSummaryInfo;
 hotStackArr:Object[] = [];
  ...
  productName: string;
  constructor(private commonServices: CommonServices, private ddrRequest:DDRRequestService, private sessionService: SessionService) { 
  ...
  getThreadDumpSummaryInfo(filePaths:string)
  {
   //  console.log(filePaths);
     ...
     summarizedTDUrl=  ipWithProd + '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/getSummaryOfThreadDumps";
...
getStatisticsTDdata(filepaths: any) {
    ...
    if(filepaths.length >= 2) {
      ...
      {
      ipWithProd=this.getURLbasedOnDC(this.threadDumpSummaryInfo[0].dcName);
      forParseThreadDumpUrl =  ipWithProd  + '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/ThreadDumpStatistics?filePath=" + fileArray;
      }
      else
      forParseThreadDumpUrl = this.getHostUrl() + '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/ThreadDumpStatistics?filePath=" + fileArray;
...
createTotalThreadInfo(data:Object)
{
 ...
 // console.log(this.totalThreadInfo);
  ...
  let fileObj:Object={
   ...
   let summarizedTDUrl= this.getHostUrl() + '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/getSummaryOfThreadDumps";
