...
import { CommonServices } from '../dumps/service/common.services'
import { Router } from '@angular/router'
// import { DdrBreadcrumbService } from '../../services/ddr-breadcrumb.service';
// import { CavTopPanelNavigationService } from '../../../../main/services/cav-top-panel-navigation.service';
import { DDRRequestService } from '../dumps/service/ddr-request.service';
// import { CavConfigService } from '../../../../main/services/cav-config.service';
import { DdrDataModelService } from "../dumps/service/ddr-data-model.service";
import { HttpClient } from '@angular/common/http';
import * as  CONSTANTS from '../dumps/breadcrumb.constants';
...
export class GcManagerComponent implements OnInit {
  ...
  host = window.location.host;
   ...
   constructor(private sessionService:  SessionService, public commonService: CommonServices, private confirmationService: ConfirmationService,
   ...
   ngOnInit() {
      ...
      this.uploadUrl = this.getHostUrl() + '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/uploadgclog?";
    ...
    if(data)
     {
      ...
      this._router.navigate(['/actions/gc-viewer']);
   ...
   getTierServerInfo(){
      ...
      var url = this._ddrData.getHostUrl() + '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/getTierServerListFromTopology?testRun=" + this.testRun + 
      '&user='+ this.userName +
...
getGcDumpInfo(isAll? : string){
 ...
 //  console.log("&tierName=", this.selectedTiers, "&serverName=", this.selectedServers, "&instanceName=", this.selectedApps)
   // gc_dump_url =  this.getHostUrl() + '/' + this.id.product + "/v1/cavisson/netdiagnostics/ddr/viewThreadDump?testRun=" + this.id.testRun + "&tierName=" + this.selectedTiers + "&serverName=" + this.selectedServers + "&instanceName=" + this.selectedApps + "&startTime=" + this.id.strStartTime + "&endTime=" + this.id.strEndTime;
    gc_dump_url = this.getHostUrl() + '/' + this.productName + "/v1/cavisson/netdiagnostics/webddr/getGcDumpList?testRun=" + this.testRun +
 ...
 createGCDumpInfo(data:any){
   ...
   let urlInfo = '&tierName=' + this.selectedTierVal + '&serverName=' + this.serverNameforGC + "&startTime=undefined&endTime=undefined&timestamp=" + '&fileName=' + filename + '&filePath=' + '-' + '&userName=' + this.userName + '&location=' + location;

    let url = this.getHostUrl() + '/' + this.productName + "/v1/cavisson/netdiagnostics/webddr/downloadGCFile?testRun=" + this.testRun + urlInfo ;
 ...
 removeFile() {
   ...
   let Gc_log_remove_url = this.getHostUrl() + '/' + this.productName + "/v1/cavisson/netdiagnostics/webddr/removeGcLogFile?testRun=" + this.testRun + '&filePath=' + this.nodeValueToDelete.filePath + '/' + this.nodeValueToDelete.fileName +  '&userName=' + this.nodeValueToDelete.userName + '&index=' + this.nodeValueToDelete.index + '&productKey=' + this.productKey;
 ...
 delete_GC_url = this.getHostUrl() + '/' + this.productName + "/v1/cavisson/netdiagnostics/ddr/removeSelectedGcFile?testRun=" + this.testRun+'&filePathList='+fullFilePathList+'&indexList='+fileIndexList + '&productKey=' + this.productKey + '&userName=' + this.userName ;
...
onRowSelect(data: any) {
 ...
 analyzeGCFile(node){
  ...
  if(node){
    analyzeUrl = this.getHostUrl() + '/' + this.productName + "/v1/cavisson/netdiagnostics/webddr/analyzeGcFile?testRun=" + this.testRun +'&filePath=' + node.filePath +'&fileName=' + node.fileName; 
  }
else
  {
   this.loading = false;
