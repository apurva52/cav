...
export class ConfigApplicationListComponent implements OnInit {
  ...
  constructor(private configApplicationService: ConfigApplicationService, private configHomeService: ConfigHomeService, private configUtilityService: ConfigUtilityService, private sessionService: SessionService, private confirmationService: ConfirmationService, private router: Router, private configTopologyService: ConfigTopologyService, breadcrumb: BreadcrumbService) { 
  ...
  cols = [
    ...
    { field: 'topoName', header: 'Topology Applied' },
	  { field: 'userName', header: 'User Name' },
	  { field: 'timeStamp', header: 'Last Updated On' },
	  { field: 'appId', header: 'ND Server Settings' },
	  { field: 'appDesc', header: 'Description' }
    ];

  ngOnInit() {
	this.breadcrumb.add({label: 'Application List', routerLink: '/nd-agent-config/application-list'});
    ...
    if(this.runningApp != ""){
      ...
      for(let i in this.selectedApplicationData){
        if(this.selectedApplicationData[i].appName == this.runningApp){
          this.configUtilityService.errorMessage("Running Application '" + this.runningApp + "'" + " cannot be deleted")
          return;
        }
      }
    }  
