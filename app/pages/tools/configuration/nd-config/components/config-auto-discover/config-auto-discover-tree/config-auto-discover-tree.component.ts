...
export class ConfigAutoDiscoverTreeComponent implements OnInit {
    ...
    isShowFileExistDialog : boolean = false;

    //To check the node count while Instrumentation
    nodesThresholdAD: number;
    isShowWarningDialog: boolean = false;
    breadcrumb: BreadcrumbService;
    constructor(private configNdAgentService: ConfigNdAgentService, private http: HttpClient, private _configKeywordsService: ConfigKeywordsService, 
        private configUtilityService: ConfigUtilityService, private configProfileService: ConfigProfileService, breadcrumbService: BreadcrumbService,
        private configHomeService: ConfigHomeService) {
        this.loadAutoDiscoverData();
	this.breadcrumb = breadcrumbService;
        this.breadcrumb.add({label: 'Auto Discover', routerLink: '/nd-agent-config/auto-discover-tree'});
    }
    ...
    saveInstrumentationFile() {
        ...
        // Count the nodes of Instrumented data 
        let countNodes = 0;
        for(let i=0; i<this.rightSideTreeData[0]['children'].length; i++)
        {   
            countNodes++;
            for(let j=0; j<this.rightSideTreeData[0]['children'][i]['children'].length; j++)
            {
                countNodes++;
                for(let k=0; k<this.rightSideTreeData[0]['children'][i]['children'][j]['children'].length; k++)
                {
                    countNodes++;
                }
            }
        }
        //If countNodes is greater than keyword than show warning dialog
        if(countNodes > this.nodesThresholdAD)
            this.isShowWarningDialog = true;
        else
            this.checkInstrumentFileExistOrNot();
   }

    saveInstrumentFileIfNotExist()
    {
	...
	this.isShowFileExistDialog = false;
	...
	//Invoke method to get value of nodesThresholdAD keyword
        this.getConfigIniFileInfo();
    }
    ...
    //This method is used to get value of nodesThresholdAD
    getConfigIniFileInfo(): void {
    this.configHomeService.getMainData()
      .subscribe(data => {
        this.nodesThresholdAD = data.nodesThresholdAD;
      }
      );
    }

  checkInstrumentFileExistOrNot()
  {
      this.isShowWarningDialog = false;
    //This service is used to fetch all file names
    this._configKeywordsService.getInstrumentationProfileXMLFileList(this.agentType).subscribe(data => {
        for(let i=0; i<data.length; i++)
        {
            if(this.instrfileName == data[i].substring(0,data[i].indexOf('.')))
            {
                this.isShowFileExistDialog = true;
                break;
            }
        }
        if(!this.isShowFileExistDialog)
        {
            this.saveInstrumentFileIfNotExist();
        }
    });
  }
}
