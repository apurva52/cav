...
export class ConfigEditAutoInstrumentationComponent implements OnInit {
    ...
    constructor(private confirmationService: ConfirmationService, private router: Router, private route: ActivatedRoute, private configNdAgentService: ConfigNdAgentService, private http: HttpClient, private _configKeywordsService: ConfigKeywordsService, private configUtilityService: ConfigUtilityService, private configProfileService: ConfigProfileService, private configHomeService: ConfigHomeService) {
    ...
    saveInstrumentationFile() {
        ...
        if(this.agentType == "Java"){
            this.instrfileNameWithExtension = this.instrfileName +".xml" + "##" + this.agentType;
        }
        else{
            this.instrfileNameWithExtension = this.instrfileName +".txt" + "##" + this.agentType;
        }
    ...
    getAgentSpecificProfiles(agentType) {
		...
		console.log("this.profileData[j].agent==>"+this.profileData[j].agent);
		console.log("this.profileData[j].profileId==>"+this.profileData[j].profileId);
		...
		else if ((agentType == "Php" || agentType == "php" || agentType == "PHP") && this.profileData[j].agent == "Php" && this.profileData[j].profileId != 666666) {
                    if (this.profileData[j].profileName == arr[i]) {
                        this.profileListItem.push({ label: arr[i], value: this.profileData[j].profileId });
                    }
                }
