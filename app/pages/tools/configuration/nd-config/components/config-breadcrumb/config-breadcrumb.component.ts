...
export class ConfigBreadcrumbComponent implements OnInit, OnDestroy {
  ...
  constructor(private router: Router, private configHomeService: ConfigHomeService,
    private configTopologyService: ConfigTopologyService, breadcrumbService: BreadcrumbService) { this.breadcrumb = breadcrumbService; }
  ...
  ngOnInit() {
    ...
    this.breadcrumbSubscription = this.router.events.pipe(filter(event => event instanceof NavigationEnd)).subscribe(event => {
      let url = event["url"];
      this.getBreadCrumbURL(url);
      sessionStorage.setItem("BreadCrumbUrl" , url)
    });

    let timerObj =  timer(30000, this.refreshIntervalTime);
    this.subscription = timerObj.subscribe(t => this.getTestInfoDetails());
  }
  ...
  sessionStorage.setItem("agentType", "");
    ...
    else if(url.startsWith(BREADCRUMB.URL.MEM_PROF)) {
      this.items.push({label: BREADCRUMB.LABEL.MEM_PROF})
    }
