...
export class ConfigImportInstrProfileComponent implements OnInit {
  ...
  constructor(private _configKeywordsService: ConfigKeywordsService, private _configUtilityService: ConfigUtilityService, private confirmationService: ConfirmationService, private configHomeService: ConfigHomeService, breadcrumbService: BreadcrumbService) {
  ...
  //Variable will true if given file name is already present.
  isExistingFile : boolean = false;

  //Array to store existing files
  existingFilesDataArr = [];

  //temp variable to show alert dialog
  tempSelectedFile: string ="";
  tempSelectedAgent: string = "";
  //This variable is used to keep previous agent while select another agent from dropdown to show alert
  changeAgents: boolean = true;

  enableMethodLevel: boolean = false;
  levels=[{label: "1", value: 1}, {label: "2", value: 2}, {label: "3", value: 3}, {label: "4", value: 4}];
  selectedLevel: string = "";

  ngOnInit() {
    this.breadcrumb.add({label: 'Instrumentation Profile Maker', routerLink: '/nd-agent-config/instrumentation-profile-maker'});
    ...
    // });
    let agentVal = ['Dot Net', 'Java', 'NodeJS', 'Php', 'Python'];
  ...
  getAgentSpecificFiles(val) {
    if (!this.changeAgents && this.editFile) {
      this.clearWindow();
    }
    this.viewInstrumentation = false;
  ...
  getFileList(path) {
    if (this.isMakeXMLFile == true) {
      ...
      this._configKeywordsService.getInstrumentationProfileXMLData(path).subscribe(data => {
        ...
        this.xmlFormat = data;
        this.xmlFormat = Prism.highlight(data, Prism.languages.markup);
        if (this.xmlFormat == "") {
  ...
  showSelectedFileData() {
    ...
    //If file is in Edit or Create mode then will display alert dialog
    if (this.editFile || this.createInstrumentation) {
      let flag = this.showAlertDialog();
      if (!flag)
        return;
    }
    this._configKeywordsService.getXMLDataFromSelectedXMLFile(this.selectedFile, this.selectedAgent).subscribe(data => {
      // const xml = beautify(data);
      this.xmlFormat = beautify(data);
      // this.xmlFormat = data;
      this.xmlFormat = Prism.highlight(this.xmlFormat, Prism.languages.markup);
    });
    this.editFile = false;
  ...
  /**Add node to tree */
  addNodeToTree(type, label, level) {
    try {
        ...
        for (let index = 0; index < mainData.length; index++) {
          if (contextMenuEvent['node']['parentPackageNode'] === mainData[index]['parentPackageNode']) {
            for (let j = 0; j < mainData[index]['children'].length; j++) {
              ...
              if (contextMenuEvent['node']['parentClassNode'] === mainData[index]['children'][j]['parentInterfaceNode']) {
                for (let k = 0; k < mainData[index]['children'][j]['children'].length; k++) {
                  if (label === mainData[index]['children'][j]['children'][k]['label']) {
                    DataArr[0]['children'][index]['children'][j]['children'].splice(k, 1);
                    break;
  ...
  createInstrumentationFile() {
    //If file is already in edit or create mode then will show alert dialog
    if (this.editFile || this.createInstrumentation) {
      let flag = this.showAlertDialog();
      if (!flag)
        return;
    ...
    if (this.selectedAgent == 'Java') {
      if (this.nodeObj['type'] === 'All') {
	    ...
	    if(this.selectedLevel != "" && this.selectedLevel != undefined) {
              this.nodeLabel = this.nodeLabel.replace(/\./g, '/') + "|" + this.selectedLevel;
            }
          }
        }
      }
      ...
      this.addNodeToTree(this.nodeObj['type'], this.nodeLabel, this.selectedLevel);
      this.addNodeDialog = false;
      this.enableMethodLevel = false;
      this.selectedLevel = '';
      this.nodeLabel = '';
  ...
  saveModifiedXMLNodes(flag) {
    ...
    this._configKeywordsService.getSelectedProfileDetails(this.selectedFile, trMsg).subscribe(data => {
      ...
      if (msg != "" && !this.isSaveAsEditedFile) {
        ...
        this.checkFileExistOrNot();	
        if(!this.isExistingFile)
          confirmMsg = "Do you want to save changes?"
        else
          confirmMsg = "Provided file name already exist. Do you want to override the existing file?" 
      }
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          ...
          for (let i = 0; i < dataToBeUploaded.length; i++) {
			...
			let methodLevel = methodList[k].level;
			uploadArray.push(`${packageName}^${interfaceName}^${methodName}@${methodLevel}#`);
		      }
		      else
			uploadArray.push(`${packageName}^${interfaceName}^${methodName}#`);
		      ...
		      if(methodName.includes("|"))
		      {
 		      	methodName = methodName.substring(0, methodName.lastIndexOf('|'));
                      	let methodLevel = methodList[k].level;
                      	uploadArray.push(`${packageName}^${className}^${methodName}@${methodLevel}`);
		      }
		      else
  ...
  deleteSelectedFile() {
	...
	//If file is in already Edit or Create mode then will show alert dialog
        if (this.editFile || this.createInstrumentation && this.isSaveAsEditedFile) {
      		let flag = this.showAlertDialog();
      		if (!flag)
        		return;
    	}
  ...
  clearWindow() {
    //If file is already in Edit or Create mode then will show a alert dialog
    if (this.editFile || this.createInstrumentation && this.isSaveAsEditedFile) {
      let flag = this.showAlertDialog();
      if (!flag)
        return;
...
//Check whether given file name is already exist or not.
checkFileExistOrNot()
{
          for(let i=0; i<this.existingFilesDataArr.length; i++)
          {
              if(this.saveFileName == this.existingFilesDataArr[i].substring(0,this.existingFilesDataArr[i].indexOf('.')))
              {
  ...
  canDeactivate() {
    if (this.TreeMenu || this.editFile || this.createInstrumentation) {
      return window.confirm('You have unsaved changes. Are you sure want to leave this page?');
    }
  ...
  showAlertDialog() {
    ...
    flag = window.confirm("You may have unsaved changes. Are you sure you want change file?");
    return flag;
  }

  /* this method is used for remove data from gui after deleting any file*/
  clearWindowForDeleteFile() {
    this.xmlFormat = "";
    this.xmlFormat = "No file selected";
    this.selectedFile = "";
    this.instrprofdata = [];
  ...
  onChange(event) {
    this.selectedFile = '';
    if (this.editFile || this.createInstrumentation) {
      const response = window.confirm("Are you sure to want the change agent?");
      if (response) {
        this.selectedAgent = event;
        this.tempSelectedAgent = event;
        this.createInstrumentation = false;
        this.editFile = false;
      }
      else {
        this.selectedAgent = this.tempSelectedAgent;
        this.selectedFile = this.tempSelectedFile;
      }
