...
//for holding operator';s value of Extract SubPart Operation for return type
  ...
  constructor(private configKeywordsService: ConfigKeywordsService, private store: Store<Object>, private route: ActivatedRoute, private configCustomDataService: ConfigCustomDataService, private configUtilityService: ConfigUtilityService, private confirmationService: ConfirmationService) {
    // 
    this.subscription = this.store.select(store => store["customData"]).subscribe(data => {
      // this.tableData = data;
    })
    ...
    this.cols = [
      ...
      {field: 'fqm', header: 'Fully Qualified Method Name'},
      {field: 'returnTypeValue', header: 'Return Type'},
      {field: 'argumentTypeValue', header: 'Argument Type Value'}
    ];
  }
  profileId: number;
  ...
  */
  ...
  saveEditData(fqmField) {
	...
	//to check if key name already exists
        for (let i = 0; i < this.tableData.length; i++) {
          if (this.tableData[i].fqm == this.methodBasedCustomData.fqm) {
            if (this.tableData[i].fqm == this.methodBasedCustomData.fqm) { 
              this.configUtilityService.errorMessage("Fully qualified method name already exist");
              return;
            }
          }
        }
...
//deleting Argument rules
  ...
  getTypeName(type){
   ...
   saveReturnRules() {
	...
	//To check existance of Header Name
        if(this.isHeaderNameExistForRetTypeEdit())
        {
          return;
        }
        this.editReturnRules = false
      ...
      //to check if Header name already exists
      if(this.isHeaderNameExistForRetType())
      {
        return;
      }
      ...
      /*********** for ADD form *******************/
        ...
        */   
      if(this.editReturnRules){
	//Check existance of Header Name
        if(this.isHeaderNameExistForRetTypeEdit())
        {
          return;
        }
  ...
  saveArgumentRules() {
	...
	//Check existance of Header Name
        if(this.isCheckHeaderNameExistForArgTypeEdit())
        {
          return;
        }
       ...
       //Check existance of Header Name
       if(this.isCheckHeaderNameExistForArgType())
       {
         return;
       }
  ...
  //to check if Header name already exists
  isHeaderNameExistForRetTypeEdit() : boolean
  {
    for (let i = 0; i < this.returnTypeData.length; i++) {
      if (this.returnTypeRules.headerName == this.selectedReturnRules[0].headerName) { }
      else if (this.returnTypeData[i].headerName == this.returnTypeRules.headerName) { 
  ...
  //to check if Header name already exists
  isCheckHeaderNameExistForArgTypeEdit() : boolean
  {
    for (let i = 0; i < this.argumentTypeData.length; i++) {
      if (this.argumentTypeRules.headerName == this.selectedArgumentRules[0].headerName) { }
      else if (this.argumentTypeData[i].headerName == this.argumentTypeRules.headerName) { 
        this.configUtilityService.errorMessage("Header name already exist");
          return true;
      }
    }
  ...
  //to check if Header name already exists
  isCheckHeaderNameExistForArgType() : boolean
  {
    for (let i = 0; i < this.argumentTypeData.length; i++) {
      if (this.argumentTypeData[i].headerName == this.argumentTypeRules.headerName) { 
        this.configUtilityService.errorMessage("Header name already exist");
          return true;
      }
    }
  ...
  //to check if Header name already exists
  isHeaderNameExistForRetType() : boolean
  {
    for (let i = 0; i < this.returnTypeData.length; i++) {
      if (this.returnTypeData[i].headerName == this.returnTypeRules.headerName) { 
        this.configUtilityService.errorMessage("Header name already exist");
          return true;
      }
    }
