...
export class IntegrationPtComponent implements OnInit {
  ...
  keywordList = ['NDEntryPointsFile', 'ndBackendNamingRulesFile', 'enableIPForNodeJS'];
  ...
  //Advance Custom Operations dropdown
  advCustomOperationsLabel: any[] = ['Return type', 'Argument', 'All', 'Default'];
  advCustomOperationsValue: any[] = ['RT', 'ARG', 'ALL', 'DEFAULT'];

  /* Assign value to advCustomOperationsList drop down */
  advCustomOperationsList :  SelectItem[];

  selectedAdvCustomOperation : any;
  selectedValueType : boolean = false;
  selectedValueTypeNum : any;

  //Variable to append paths in ndsettings.txt
  enableIPForNodeJS : boolean = false;

  //To show/hide filters
  isIntPntFilters : boolean = true;
  isEnableColumnFilter : boolean;

  cols = [
    { field: 'checkbox', header: '' },
    { field: 'type', header: 'Integration Point Type' },
  ...
  constructor(private route: ActivatedRoute, private configKeywordsService: ConfigKeywordsService, private configUtilityService: ConfigUtilityService, private confirmationService: ConfirmationService, private store: Store<KeywordList>) {
    ...
    this.subscription = this.store.select(store => store["keywordData"]).subscribe(data => {
      ...
      this.integrationPoints = cloneObject(keywordDataVal);
    });
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe((params: Params) => {
      ...
      if (this.profileId == 1 || this.profileId == 777777 || this.profileId == 888888 || this.profileId == 666666)
    ...
    this.advCustomOperationsList = ConfigUiUtility.createListWithKeyValue(this.advCustomOperationsLabel, this.advCustomOperationsValue);

    //Set value for keyword enableIPForNodeJS
    this.setEnableIPForNodeJSKeyword();
  }
  ...
  saveAddIntegrationPTDetection(): void {
    ...
    if (this.agentType == "Java") {
      this.addIPDetectionDetail.module = "-";
        ...
        //For Custom Backend
        if(this.addIPDetectionDetail.backendTypeId == 46)
        {
          if(this.selectedValueType == false)
          {
            this.selectedValueTypeNum = 0;
          }
          else
          {
            this.selectedValueTypeNum = 1;
          }

          if(this.selectedAdvCustomOperation == 'ARG' || this.selectedAdvCustomOperation == 'ALL')
          {
            this.addIPDetectionDetail.name = this.addIPDetectionDetail.name + ':' + this.selectedAdvCustomOperation + ':' + this.addIPDetectionDetail.argumentIndex + ':' + this.selectedValueTypeNum;
          }
          else
          {
            this.addIPDetectionDetail.argumentIndex = -1;
            this.addIPDetectionDetail.name = this.addIPDetectionDetail.name + ':' + this.selectedAdvCustomOperation + ':' + this.addIPDetectionDetail.argumentIndex + ':' + this.selectedValueTypeNum;
            // endPointData.name = data.name + ':' + this.selectedAdvCustomOperation + ':'  + endPointData.argumentIndex + ':' + this.selectedValueTypeNum;
          }
        }
    ...
    this.selectedAdvCustomOperation = '';
    this.selectedValueType = false;
...
*/
  openDialiogForEntry(data){
    if(data.id == 46 )
    {
      for(let i=0; i<data.lstEndPoints.length; i++)
        data.lstEndPoints[i].labelOfAdvCustom = data.lstEndPoints[i].name.split(":")[0]
    }
    else
    {
      for(let i=0; i<data.lstEndPoints.length; i++)
        data.lstEndPoints[i].labelOfAdvCustom = '';
    }

    this.selectedIpDetectionData = data;
  ...
  saveIntegrationDetail() {
    ...
    this.namingRuleAndExitPoint.projectId = this.integrationDetail.namingRule.projectId;
    this.namingRuleAndExitPoint.instanceId = this.integrationDetail.namingRule.instanceId;
    this.namingRuleAndExitPoint.databaseId = this.integrationDetail.namingRule.databaseId;
    this.namingRuleAndExitPoint.operation = this.integrationDetail.namingRule.operation;
    this.namingRuleAndExitPoint.vendorName = this.integrationDetail.namingRule.vendorName;
    this.namingRuleAndExitPoint.brokerId = this.integrationDetail.namingRule.brokerId;
    this.namingRuleAndExitPoint.subId = this.integrationDetail.namingRule.subId;
    this.namingRuleAndExitPoint.databaseName = this.integrationDetail.namingRule.databaseName;
    this.namingRuleAndExitPoint.protocol = this.integrationDetail.namingRule.protocol;
    this.namingRuleAndExitPoint.queryParameter = this.integrationDetail.namingRule.queryParameter;
  ...
  setNamingRuleAndExitPointData(backendTableInfo: NamingRule, data: NamingRuleAndExitPoint) {
    ...
    backendTableInfo.projectId = data.projectId;
    backendTableInfo.instanceId = data.instanceId;
    backendTableInfo.databaseId = data.databaseId;
    backendTableInfo.operation = data.operation;
    backendTableInfo.vendorName = data.vendorName;
    backendTableInfo.brokerId = data.brokerId;
    backendTableInfo.subId = data.subId;
  }
  ...
  saveIntegrationPointOnFile() {
    this.configKeywordsService.saveIntegrationPointData(this.profileId)
	  ...
	  //To append NDEntryPointFile.txt and BackendNamingRule.txt path in ndsettings.txt file	
	  if(this.agentType == 'NodeJS')
	  {
	  	if(this.enableIPForNodeJS)
                	that.integrationPoints['enableIPForNodeJS'].value = 1;
                else
                	that.integrationPoints['enableIPForNodeJS'].value = 0;
          }
 ...
 */
  ...
  //To set value for variable enableIPForNodeJS
  setEnableIPForNodeJSKeyword()
  {
    if(this.integrationPoints['enableIPForNodeJS'].value == '1')
      this.enableIPForNodeJS = true;
    else
      this.enableIPForNodeJS = false;
  }

  //Method to show/hide column filters
  showHideColumnFilter()
  {
    if(this.isIntPntFilters)
    {
      this.isEnableColumnFilter = true;
      this.isIntPntFilters = false;
    }
    else
    {
      this.isEnableColumnFilter = false;
      this.isIntPntFilters = true;
    }
  } 
