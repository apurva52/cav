import { Component, OnInit, Output, EventEmitter, PipeTransform, Pipe ,Inject, ViewChild} from '@angular/core';
...
export class HTTPBTConfigurationComponent implements OnInit {
  ...
  arrStringLabelReturnType: any[] = ['Equals', 'Not equals', 'Contains', 'Starts with', 'Ends with', 'Exception'];
  arrStringValueReturnType: any[] = ['EQUALS', 'NOT_EQUALS', 'CONTAINS', 'STARTS_WITH', 'ENDS_WITH', 'EXCEPTION'];


  arrNumericLabelReturnType: any[] = ['Equals', 'Not equals', 'Less than', 'Greater than', 'Less than equals to', 'Greater than equals to', 'Exception'];
  arrNumericValueReturnType: any[] = ['EQ', 'NE', 'LT', 'GT', 'LE', 'GE', 'EXCEPTION'];

  arrCharLabelReturnType: any[] = ['Exception', 'Equals', 'Not equals'];
  arrCharValueReturnType: any[] = ['EXCEPTION', 'EQ', 'NE'];

  arrBooleanLabelReturnType: any[] = ['True', 'False', 'Exception'];
  arrBooleanValueReturnType: any[] = ['TRUE', 'FALSE', 'EXCEPTION'];
  ...
  addResReqHeaderDialogFlag: boolean = false;
  ...
  //BT name for method Invocation
  btNameForInvocation : string;

  //To store selected previous radio button while edit Invocation 
  editBTType : any;

  //To copyData of btPattern
  copyData:any = {};

  /* Assign  dropdown values to traceRequestList */
  traceRequestList: SelectItem[];

  /* Assign  dropdown values to instrumentLevelList */
  instrumentLevelList: SelectItem[];

  //Assign dropdown value to Log Level.
  logLevelList: SelectItem[];
  ...
  constructor(private route: ActivatedRoute,
    ...
    this.traceRequestList = [];
    let traceRequestLabel = ['Disable, do not capture body of request & response', 'Capture only request body', 'Capture only response body', 'Capture both request and response  body'];
    let traceRequestValue = [0, 1, 2, 3];
    this.traceRequestList = ConfigUiUtility.createListWithKeyValue(traceRequestLabel, traceRequestValue);

    this.instrumentLevelList = [];
    let instrumentLevelLabel = ['1', '2', '3', '4'];
    let instrumentLevelValue = [1, 2, 3, 4];
    this.instrumentLevelList = ConfigUiUtility.createListWithKeyValue(instrumentLevelLabel, instrumentLevelValue);

    //For Log Level
    this.logLevelList = [];
    let logLevelLabel = ['None', 'Error ', 'Warn', 'Info', 'Debug', 'Trace', 'All'];
    let logLevelValue = ['NONE', 'ERROR ', 'WARN', 'INFO', 'DEBUG', 'TRACE', 'ALL'];
    this.logLevelList = ConfigUiUtility.createListWithKeyValue(logLevelLabel, logLevelValue);
    ...
    this.invocationRulesInfo = [];

    this.configHomeService.getBTMethodDataFromAD$.subscribe(val => {
      this.flagAD = val;
    });
    // TO OPEN METHODBT FROM AUTO DISCOVER
  ...
  ngOnInit() {
    this.document.body.classList.add('profile-dialog');
  ...
  getKeywordData() {
    ...
    if (this.configKeywordsService.keywordData != undefined) {
      ...
      this.subscription = this.store.select(store => store["keywordData"]).subscribe(data => {
        ...
        keywordData = cloneObject(keywordDataVal);
      });
    }
  ...
  setSelectedValueOfBT(value) {
    ...
    let filePath = '';
    this.BusinessTransGlobalPattern = cloneObject(this.BusinessTransGlobalPattern);
  ...
  loadBTPatternData(): void {
    this.configKeywordsService.getBusinessTransPatternData(this.profileId).subscribe(data => {
      ...
      for (let row of data) {
        if (row.btName == "" || row.btName == undefined || row.btName == null) {
          row.btName = "-"
          row.matchType = '-'
          row.urlName = '-'
          row.include = '-'
          row.slowTransaction = '-'
          row.verySlowTransaction = '-'
          row.reqParamKeyVal = '-'
          row.reqMethod = '-'
          row.headerKeyValue = '-'
        }
  ...
  /**
   * Earlier: This method is used to save the newly added BT Pattern
   * Now: This method will modify the values of the BTPattern data inputted by user
   */
  savePattern(): void {

    //check for blank fields
    if (this.btHttpHeadersInfo.length == 0 && this.btResponseHeadersInfo.length == 0 && this.httpBodyInfo.length == 0 && this.businessTransMethodInfo.length == 0) {
      //if bt name, url, and mode are blank then do not allow to save
      if (this.businessTransPatternDetail.btName == "" || this.businessTransPatternDetail.btName == null || this.businessTransPatternDetail.urlName == "" || this.businessTransPatternDetail.urlName == null || this.businessTransPatternDetail.matchType == undefined || this.businessTransPatternDetail.matchType == null) {
        this.configUtilityService.errorMessage("Pattern details are missing");
        return;
      }
    }
    ...
    //validation check for Threshold Counter
    if(this.businessTransPatternDetail.threshold >= 1001 || this.businessTransPatternDetail.threshold <= 0)
    {
      this.configUtilityService.errorMessage("Threshold Counter must be in range between 0 to 1001");
      return;
    }
    ...
    if (this.businessTransPatternInfo.length > 0) {
      for (let i = 0; i < this.businessTransPatternInfo.length; i++) {
        ...
        if (this.businessTransPatternDetail.btName != "" && this.businessTransPatternDetail.btName != null && this.businessTransPatternDetail.btName != undefined) {
  ...
  //IN AD, there is no need to add BTPattern row, so creating a new function so that default values of BTPattern could not be created
  savePatternRow() {
    let id = this.parentBtId
    let businessTransPatternDetail = this.businessTransPatternDetail;
    this.configKeywordsService.addBusinessTransPattern(businessTransPatternDetail, this.profileId, id)
      .subscribe(data => {
        // The below method is called to set all values that contains "-" to null
        // this.methodToSetValuesForGUI(data);
        ...
        //If observables is blank then no child is inserted so insert entries in table
  ...
  openAddPatternDialog() {
    ...
    this.btNameForInvocation = '';
  }


  defaultBTPatternValues() {
    ...
    this.businessTransPatternDetail.threshold = 50;
    this.businessTransPatternDetail.traceRequest = 0;
    this.businessTransPatternDetail.instrumentLevel = 1;
    this.businessTransPatternDetail.logLevel = 'NONE';
  ...
  editPatternDialog(): void {
    ...
    this.businessTransPatternDetail.id = this.selectedPatternData[0].id
    this.businessTransPatternDetail.btRuleId = this.selectedPatternData[0].btRuleId
    ...
    //get list of associated bt http request hdr(if any)
  ...
  methodToSetValuesForGUI(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if(data[i].showAllBT != null)
      {
      if(data[i].showAllBT.includes("|"))
      {
        data[i].showAllBT = data[i].showAllBT.replace(/[|]/g,',');
        data[i].showAllBT = data[i].showAllBT.replace(/[, ]+/g, ', ')
        if(data[i].showAllBT.startsWith(","))
        {
          data[i].showAllBT = data[i].showAllBT.substring(1)
        }
        if(data[i].showAllBT.endsWith(","))
        {
          data[i].showAllBT = data[i].showAllBT.substring(0,data[i].showAllBT.length-1)
        }
        if(data[i].showAllBT.endsWith(", "))
       	{
          data[i].showAllBT = data[i].showAllBT.substring(0,data[i].showAllBT.length-2)
        }
      }
     }
    }
  ...
  editPattern(): void {
    ...
    if (this.businessTransPatternDetail.urlName != "" && this.businessTransPatternDetail.urlName != null && this.businessTransPatternDetail.urlName != undefined) {
      for (let i = 0; i < this.businessTransPatternInfo.length; i++) {
        if (this.selectedPatternData[0] != this.businessTransPatternInfo[i]) {
          if (this.businessTransPatternDetail.urlName == this.businessTransPatternInfo[i].urlName
            && this.businessTransPatternDetail.matchType == this.businessTransPatternInfo[i].matchType
  ...
  downloadReports(reports: string) {
    ...
    let arrFieldName = { "0": "showAllBT", "1": "matchType", "2": "urlName", "3": "include", "4": "slowTransaction", "5": "verySlowTransaction", "6": "slowDynamicThreshold", "7": "verySlowDynamicThreshold", "8": "paramKeyValue", "9": "reqMethod", "10": "headerKeyValue" };
    let object =
    {
  ...
  readGlobalThresholdFile() {
    ...
    this.configKeywordsService.readGlobalThresholdFile(this.profileId).subscribe(data => {
      this.globalFileData = data.split("|")
 ...
 */
  ...
  changeOpertionType(type) {
    ...
    if (type == "object/string") {
      ...
      this.returnOperationList = [];
      this.configUtilityService.errorMessage("For BT Return Type, FQM having return type void can't be accepted");
    }
  }
  ...
  */
  ...
  editMethodTrans(): void {
    ...
    if (this.businessTransMethodDetail.enableArgumentType && ((this.businessTransMethodDetail.methodInvocation == null || this.businessTransMethodDetail.methodInvocation == '-1'))) {
      ...
      // this.btMethodRulesDetail.btMethodRuleId = this.methodArgRulesInfo[0].btMethodRuleId;
      this.methodRulesInfo = [];
      this.methodInvocationRulesInfo = [];
      this.invocationRulesInfo = [];
      this.enableArgumentType = "argument";
      this.editBTType = "argument";
      this.btNameForInvocation = '';
    }
     ...
     // this.validateArgAndGetArgumentsNumberList('INVOCATION');
      ...
      // this.btMethodRulesDetail.btMethodRuleId = this.methodInvocationRulesInfo[0].btMethodRuleId;
      this.methodArgRulesInfo = [];
      this.methodRulesInfo = [];
      this.invocationRulesInfo = [];
      this.enableArgumentType = "invocation";
      this.editBTType = "invocation";
      this.btNameForInvocation = '';
    }
    else if (this.businessTransMethodDetail.enableArgumentType && this.businessTransMethodDetail.methodInvocation == '-2') {
     // this.validateArgAndGetArgumentsNumberList('INVOCATION');
      this.invocationRulesInfo = this.selectedbusinessTransMethod[0].rules;
      this.btNameForInvocation = this.invocationRulesInfo[0].btName;
      this.methodArgRulesInfo = [];
      this.methodRulesInfo = [];
      this.methodInvocationRulesInfo = [];
      this.enableArgumentType = "methodInvocation";
      this.editBTType = "methodInvocation";
    }
    else {
      this.methodRulesInfo = this.selectedbusinessTransMethod[0].rules;
      // this.btMethodRulesDetail.btMethodRuleId = this.methodRulesInfo[0].btMethodRuleId;
      this.methodArgRulesInfo = [];
      this.methodInvocationRulesInfo = [];
      this.invocationRulesInfo = [];
      this.enableArgumentType = "returnType";
      this.editBTType = "returnType";
      this.btNameForInvocation = '';
    }
  ...
  editMethod(): void {
    if (this.enableArgumentType == "returnType") {
      ...
      if (this.methodRulesInfo != []) {
        ...
        this.methodArgRulesInfo = [];
        //If return type is selected then, to delete all the rules from Return Invocation table
        ...
        for (let index in this.invocationRulesInfo) {
          if (this.invocationRulesInfo[index].btMethodRuleId != null)
                this.methodBtTypeDelete.push(this.invocationRulesInfo[index].btMethodRuleId);
        }
        this.invocationRulesInfo = [];
      }
    }
      ...
      this.businessTransMethodDetail.enableArgumentType = true;
      this.businessTransMethodDetail.methodInvocation = '1';
      this.businessTransMethodDetail.methodInvocationIndex = 0;
      this.businessTransMethodDetail.argumentIndex = 0;
    ...
    else if (this.enableArgumentType == "methodInvocation") {
      this.businessTransMethodDetail.enableArgumentType = true;
      this.businessTransMethodDetail.rules = this.invocationRulesInfo;
      this.businessTransMethodDetail.methodInvocation = '-2';
      this.businessTransMethodDetail.methodInvocationIndex = 0;
      this.businessTransMethodDetail.argumentIndex = 0;
      //If argument type is selected then, to delete all the rules from return table
      if (this.methodRulesInfo != []) {
        ...
        this.methodArgRulesInfo = [];
        //If argument type is selected then, to delete all the rules from Return Invocation table
        for (let index in this.methodInvocationRulesInfo) {
          if (this.methodInvocationRulesInfo[index].btMethodRuleId != null)
            this.methodBtTypeDelete.push(this.methodInvocationRulesInfo[index].btMethodRuleId)
        }
    ...
    if (this.enableArgumentType == "invocation" || this.enableArgumentType == "methodInvocation") {
    ...
    if (this.enableArgumentType == "methodInvocation" && this.invocationRulesInfo.length == 0) {
      this.configUtilityService.errorMessage("Provide invocation type settings");
      return;
    }
  ...
  //For checking FQM 
  saveRules() {
    if (this.btMethodRulesDetail.btName == undefined || this.btMethodRulesDetail.btName == "") {
      this.configUtilityService.errorMessage("Enter BT Name");
      return;
    }
    else if ( this.btMethodRulesDetail.operationName == undefined || this.btMethodRulesDetail.operationName == "")
    {
      this.configUtilityService.errorMessage("Select Match Criteria");
      return;
    }
    else if (this.btMethodRulesDetail.value == undefined || this.btMethodRulesDetail.value == "")
    {
      this.configUtilityService.errorMessage("Enter Match Value");
      return;
    }

    /** if Fqm has void return type then can not add*/
  ...
  saveADDEditMethodTrans(fqmField): void {
    ...
    if(this.enableArgumentType == 'methodInvocation')
    {
      let id : number;
      let flag = false;      
      let obj = new RulesData();

      //To handle edit of BT based on method Invocation
      if(!this.isNewMethod)
      {
        if(this.editBTType !=  "methodInvocation"){
        }
        else if(this.editBTType ==  "methodInvocation"){
          id = +this.invocationRulesInfo[0].btMethodRuleId;
          flag = true;
          }
      }
      if(this.isNewMethod)
      ...
      this.invocationRulesInfo =[];
      obj.btName = this.btNameForInvocation;
      obj.operationName = "INVOCATION";
      obj.value = 'NA'
      if(flag){
        obj.btMethodRuleId = id;
      }
      this.invocationRulesInfo.push(obj);
    }
  ...
  modifyData(val) {
    if (val.enableArgumentType == true && (val.methodInvocation == null || val.methodInvocation == '-1')) {
      ...
      val.capturingType = "Return Invocation";
    }
    else if (val.enableArgumentType == true && val.methodInvocation == '-2') {
  ...
  saveInvocationRules() {
    ...
    {
      this.configUtilityService.errorMessage("Enter Method Name");
      return;
    }
  ...
  saveHttpHeaders() {
    ...
    this.configKeywordsService.addBtHttpHeaders(this.btHttpHeadersDetail, this.profileId).subscribe(data => {
      ...
      this.configUtilityService.successMessage(addMessage);
      this.modifyRequestData(this.btHttpHeadersInfo);
  ...
  */
  ...
  saveConditions() {
  if (!(this.headerConditionDetail.operation=='VALUE' || this.headerConditionDetail.operation=='') &&(this.headerConditionDetail.btName == undefined || this.headerConditionDetail.btName == "")) { 
      this.configUtilityService.errorMessage("Enter BT Name");
      return;
  }
  else if (this.headerConditionDetail.operation == undefined || this.headerConditionDetail.operation == "")
  {
      this.configUtilityService.errorMessage("Select operation");
      return;
  }
  else if (!(this.headerConditionDetail.operation=='VALUE' || this.headerConditionDetail.operation =='OCCURS' || this.headerConditionDetail.operation=='') && (this.headerConditionDetail.hdrValue == undefined || this.headerConditionDetail.hdrValue == ""))
  {
      this.configUtilityService.errorMessage("Enter Pattern");
      return;
  }
  ...
  openFileManager() {
    // this.uploadFile("C:\\Users\\himanshu.kumar\\Desktop\\btpattern.txt")
 ...
 ..\ProductUI\gui\src\app\modules\file-explorer\components\config-nd-file-explorer\ */
  ...
  openResHeader() {
    ...
    this.addResReqHeaderDialogFlag = true;
  ...
  saveResponseHeaders() {
    ...
    this.configKeywordsService.addBtResponseHeaders(this.btResponseHeadersDetail, this.profileId).subscribe(data => {
      ...
      this.configUtilityService.successMessage(addMessage);
      this.modifyResponseData(this.btResponseHeadersInfo);

    });
    this.addResReqHeaderDialogFlag = false;
  }
  ...
  */
    ...
    this.configKeywordsService.deleteResponseHdrConditions(this.responseHdrDelete).subscribe(data => {
      ...
      this.configKeywordsService.editBTResponseHeaders(this.btResponseHeadersDetail).subscribe(data => {
        ...
        this.btResponseHeadersInfo.map(function (val) {
          ...
          // that.modifyData(val);
          if (val.conditions != null && val.conditions.length != 0) {
            let btNames = that.getResponseBtNames(val.conditions);
            val.hdrBtNames = btNames
          }
          else {
            val.hdrBtNames = "NA"
  ...
  saveResponseConditions() {
    if(!(this.resHeaderConditionDetail.operation == 'VALUE' || this.resHeaderConditionDetail.operation == '') && (this.resHeaderConditionDetail.btName == undefined || this.resHeaderConditionDetail.btName == "")) {
      this.configUtilityService.errorMessage("Enter BT Name");
      return;
    }
    else if (this.resHeaderConditionDetail.operation == undefined || this.resHeaderConditionDetail.operation == "")
    {
      this.configUtilityService.errorMessage("Select operation");
      return;
    }
	  else if (!(this.resHeaderConditionDetail.operation == 'VALUE' || this.resHeaderConditionDetail.operation == 'OCCURS' || this.resHeaderConditionDetail.operation == '') && (this.resHeaderConditionDetail.hdrValue == undefined || this.resHeaderConditionDetail.hdrValue == ""))
    {
      this.configUtilityService.errorMessage("Enter Pattern");
      return;
    }
  ...
  deleteConditionFromTable2(arrResIndex: any[]): void {
    ...
    for (let index in arrResIndex) {
      rowIndex.push(this.getResponseCondIndex(arrResIndex[index]));
    }
    this.resHeaderConditionInfo = deleteMany(this.resHeaderConditionInfo, rowIndex);
  }

  /**This method returns selected condition row on the basis of selected row */
  getResponseCondIndex(appId: any): number {
  ...
  */
  ...
  //Check void return type for BT Rule Based on Return Type
  checkVoidReturnType()
	{
	  let returnType = this.getTypeReturnType(this.businessTransMethodDetail.fqm);
      if (returnType == 'void') {
...
//To show tooltip for BT name
  showTooltipForBTName(index)
  { 
    let arrshowAllBT = [];
    arrshowAllBT = this.copyData[index]['showAllBT'].split("|");
    let patternBT = arrshowAllBT[0];
    let  methodBT = arrshowAllBT[1].substring(0, arrshowAllBT[1].length-1);
    let reqHeaderBT = arrshowAllBT[2].substring(0, arrshowAllBT[2].length-1);
    let resHeaderBT = arrshowAllBT[3].substring(0, arrshowAllBT[3].length-1);
    let reqBodyBT = arrshowAllBT[4].substring(0, arrshowAllBT[4].length-1);
    if(this.agentType == 'Java')
      return `Pattern BT : ${patternBT} \n Method BT : ${methodBT} \n Req Header BT : ${reqHeaderBT} \n Res Header BT : ${resHeaderBT}`;
    else
      return `Pattern BT : ${patternBT} \n Req Header BT : ${reqHeaderBT} \n Res Header BT : ${resHeaderBT} \n Req Body BT : ${reqBodyBT}`;
  }

  //Omit all characters except numeric and comma. 
  omit_special_alpha_char(event)
  {   
    var k;  
    k = event.charCode;
