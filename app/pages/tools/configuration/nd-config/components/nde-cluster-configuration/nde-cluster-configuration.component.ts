...
import { Messages, descMsg, addMessage, editMessage, MessageForDuplicateEntry } from '../../constants/config-constant'
...
export class NDEClusterConfiguration implements OnInit, OnDestroy {
  ...
  constructor(private configUtilityService: ConfigUtilityService, private configKeywordsService: ConfigKeywordsService, private confirmationService: ConfirmationService, private configHomeService: ConfigHomeService, breadcrumbService: BreadcrumbService) {
	this.breadcrumb = breadcrumbService
  }
  cols = [
    {field: 'checkbox', header: '' },
    {field: 'name', header: 'NDE Server'},
    {field: 'ip', header: 'IP'},
    {field: 'port', header: 'Port'},
    {field: 'wsPort', header: 'Ws Port'},
    {field: 'wssPort', header: 'WSS Port'},
  ];

  column = [
    {field: 'checkbox', header: '' },
    {field: 'nde', header: 'NDE Server'},
    {field: 'tierGroup', header: 'Tier Group(s)'}
  ];

  ngOnInit(): void {
	this.breadcrumb.add({label: 'NDE Cluster Configuration', routerLink: '/nd-agent-config/nde-cluster-config'});
  ...
  editNDE() {
    ...
    this.configKeywordsService.editNDEData(this.ndeData).subscribe(data => {
      ...
      if(data.dupEntryMsg == 'DuplicateEntry')
        this.configUtilityService.errorMessage(MessageForDuplicateEntry);
      else
  ...
  //Help icon contents method
  sendHelpNotificationForNDEServer() {
    this.configKeywordsService.getHelpContent("Left Panel", "NDE Server", "");
  }
  
    sendHelpNotificationForNDERouting() {
    this.configKeywordsService.getHelpContent("Left Panel", "NDE Routing", "");
  }
}
