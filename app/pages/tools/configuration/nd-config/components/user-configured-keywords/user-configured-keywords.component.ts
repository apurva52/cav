...
export class UserConfiguredKeywordComponent implements OnInit {
  ...
  cols = [
    ...
    column = [
      { field: 'keyName', header: 'Name' },
      { field: 'defaultValue', header: 'Default Value' },
      { field: 'agentMode', header: 'Supported Agent(s)' },
      { field: 'desc', header: 'Description' }
      ];

  constructor(private configKeywordsService: ConfigKeywordsService, private configUtilityService: ConfigUtilityService, private confirmationService: ConfirmationService, private pipeForType: PipeForType, breadcrumbService: BreadcrumbService) {
	this.breadcrumb = breadcrumbService;
  }


  ngOnInit() {
	this.breadcrumb.add({label: 'User Configured Settings', routerLink: '/nd-agent-config/user-configured-keywords'});
  ...
  loadKeywordType() {
    ...
    this.keywordTypeList = this.createListWithKeyValue(typeLabel, typeVal);
  }
  ...
  private addUserKeywords() {
    ...
    for (let i = 0; i < this.usrConfiguredKeyList.length; i++) {
      if (this.usrConfiguredKeyList[i].keyName == this.usrConfiguredKeyDetail.keyName) {
  ...
  private addNDCKeywords() {
    ...
    for (let i = 0; i < this.usrConfiguredNDCKeyList.length; i++) {
      if (this.usrConfiguredNDCKeyList[i].keyName == this.usrConfiguredNDCKeyDetail.keyName) {
        this.configUtilityService.errorMessage("User Configured Settings already exists");
        return;
      }
    }
  ...
  */
  ...
  createListWithKeyValue(arrLabel: string[], arrValue: any): SelectItem[] {
    let selectItemList = [];
